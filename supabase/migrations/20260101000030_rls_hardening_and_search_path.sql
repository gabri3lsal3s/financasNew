...
begin
  ...
  if exists (select 1 from information_schema.tables where table_schema = 'public' and table_name = 'recurrence_skips') then
    ...
    execute 'create policy "recurrence_skips_all_own" on public.recurrence_skips for all using (exists (select 1 from public.recurrences r where r.id = recurrence_id and (select auth.uid()) = r.user_id and public.is_current_user_active())) with check (exists (select 1 from public.recurrences r where r.id = recurrence_id and (select auth.uid()) = r.user_id and public.is_current_user_active()));';
  end if;
end $$;
