...
- **Arquitetura de Rentabilidade Adaptativa na UI & Relatórios:**
  ...
  - O Relatório Executivo em A4/PDF (`PortfolioExecutiveReport`) integra a rentabilidade adaptativa oficial e a TIR na síntese executiva. Tabelas especializadas por classe de ativo (`ReportClassTables`) exibem colunas distintas por categoria: Ações/FIIs com Ticker, Setor, Qtd, PM, Cotação, Total, Var. Cota % e Retorno Total %; Internacional com Cotação USD e câmbio; **Renda Fixa com a taxa contratada e o vencimento** — a coluna "Indexador / Vencimento" exibe a taxa formatada (`formatFixedIncomeRateLabel`, em `domain/portfolio/fixed-income.ts`) e o vencimento em sub-linha, nunca omitindo a informação quando `fixed_income_metadata` estiver presente.
...
11. **Recomendações no Investment Wizard & Soberania da Quantidade:**
    - **Com saldo em Caixa ($> 0$):** os cards calculam a quantidade de cotas que cabe no saldo disponível ($\min(\lfloor\text{gapBRL} / \text{preço}\rfloor, \lfloor\text{caixa} / \text{preço}\rfloor)$) e exibem `"Cabe no caixa: X cota(s) (~R$ Y) · Meta: W%"`. Se o saldo for insuficiente para 1 cota, exibem `"Saldo em caixa insuficiente para 1 cota · Meta: W%"`. Ao clicar, o formulário já inicia preenchido com essas cotas (ou vazio se saldo insuficiente) e a sincronização com o caixa configurada.
    - **Sem saldo em Caixa (ou R$ 0,00):** os cards exibem `"Meta: W%"` (sem valor monetário de déficit) e, ao clicar, a quantidade no formulário vem **em branco** (`""`), permitindo digitação livre pelo usuário.
