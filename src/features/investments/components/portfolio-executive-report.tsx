import { FileText, PieChart, Printer, TrendingUp } from "lucide-react";
 ...
 * Usa Modal como preview interativo e PrintSheet para injeção de impressão.
 */
export function PortfolioExecutiveReport({
  ...
  const reportContent = (
    <div className="print-area flex flex-col gap-6 bg-surface text-foreground w-full max-w-full overflow-hidden print:overflow-visible">
      ...
      <section aria-label="Indicadores principais" className="grid grid-cols-2 gap-3 sm:grid-cols-4">
      ...
      <section aria-label="Distribuição por Classe" className="flex flex-col gap-3">
      ...
      <section aria-label="Custódia Consolidada" className="flex flex-col gap-3">
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="bg-surface-hover/60 border-b border-border text-muted-foreground">
                ...
                <th className="py-2.5 px-3 font-semibold">Classe</th>
                <th className="py-2.5 px-3 font-semibold text-right">Qtd</th>
                <th className="py-2.5 px-3 font-semibold text-right">P. Médio</th>
                <th className="py-2.5 px-3 font-semibold text-right">Cotação</th>
                <th className="py-2.5 px-3 font-semibold text-right">Valor Atual</th>
                <th className="py-2.5 px-3 font-semibold text-right">Rentab.</th>
                <th className="py-2.5 px-3 font-semibold text-right">% Cart.</th>
              </tr>
            </thead>
              ...
              {investmentRows.map((r) => {
                ...
                return (
                  <tr key={r.assetId} className="hover:bg-surface-hover/30 transition-colors">
                    <td className="py-2.5 px-3 font-mono font-bold text-foreground">{r.ticker}</td>
      ...
      {/* Rodapé */}
      <footer className="mt-2 pt-4 border-t border-border flex items-center justify-between text-[11px] text-muted-foreground">
  ...
  return (
    <>
      <Modal
        ...
        description="Visualização consolidada em formato A4 pronta para impressão ou exportação em PDF."
        size="xl"
      >
      ...
      {/* Portal de impressão: injetado no body para print @media */}
