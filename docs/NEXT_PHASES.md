...
> **Status:** v1.29 — proposta oficial elaborada após auditoria completa da base de código (2026-08-18). **v1.29** registra a **implementação e conclusão da Fase 34 — Importação e Reconciliação Inteligente de Extratos Bancários & Transações Gerais (Conta Corrente / PIX / TED)** (2026-08-18 — ver `ROADMAP.md` §3): todas as 4 etapas concluídas — motor puro `domain/reconciliation/bank-reconciliation.ts` com `reconcileBankTransactions`, isolando despesas à vista e receitas com scoring multidimensional e predição de categorias; mitigação pura das 6 fragilidades financeiras (anti double-counting de faturas com `isCreditCardPayment`, anti inflação com `isInternalTransferOrInvestment`, detecção de entradas com `isLikelyIncomeDescription`, reconhecimento de estornos e suporte a colunas `D/C`); migração `20260101000017_bank_statement_import.sql` com índice único condicional `idx_incomes_user_statement_hash` e RPC atômico `import_bank_transactions`; mutação TanStack `useImportBankTransactions`; e interface em 3 passos com `BankStatementImportDialog` integrado em `TransactionListPage`. Suíte **1.317 testes / 165 arquivos** (100% verde).
> **v1.28** registra o **Aprimoramento do Quick-Paste com NLP e Extrator de Entidades Financeiras** e a **proposta da Fase 34 — Importação e Reconciliação Inteligente de Extratos Bancários & Transações Gerais (Conta Corrente / PIX / TED)** (ver `ROADMAP.md` §3): importação universal multi-formato (OFX SGML/XML bancário nativo, CSV com suporte a colunas `D/C` e Quick-Paste com suporte a linguagem natural), motor com blindagem estrita contra as 6 fragilidades financeiras (anti double-counting de faturas, anti inflação de transferências/aportes, isolamento de saídas à vista), reconciliação bidirecional (alertando despesas e receitas órfãs no app) e gravação transacional atômica via RPC `import_bank_transactions` com deduplicação ordinal SHA-256. Suíte **1305 testes / 162 arquivos** (100% verde).
> **v1.27** registra a **proposta da Fase 34 — Importação e Reconciliação Inteligente de Extratos Bancários & Transações Gerais (Conta Corrente / PIX / TED)** (ver `ROADMAP.md` §3): importação universal multi-formato (OFX SGML/XML bancário nativo, CSV com suporte a colunas `D/C` e Quick-Paste), motor com blindagem estrita contra as 6 fragilidades financeiras (anti double-counting de faturas, anti inflação de transferências/aportes, isolamento de saídas à vista), reconciliação bidirecional (alertando despesas e receitas órfãs no app) e gravação transacional atômica via RPC `import_bank_transactions` com deduplicação ordinal SHA-256.
> **v1.26** registra a **implementação e conclusão da Fase 30 — Importação e Reconciliação Inteligente de Faturas de Cartão (F30)** (2026-08-18 — ver `ROADMAP.md` §3 e `STATEMENT_RECONCILIATION_PLAN.md`): todas as 4 etapas concluídas — motor puro `domain/reconciliation` (CSV multi-encoding com PapaParse, OFX SGML/XML nativo em TS, Quick-Paste, sniffer estatístico, scoring multidimensional 0–100 e deduplicação ordinal SHA-256), migration `0015` com RPC transacional idempotente `import_statement_expenses`, diálogo em 3 passos com Obsidian Glass (`StatementImportDialog`), integração na `CardsPage` e suíte completa de testes (**1237 testes / 151 arquivos**, 100% verde).
> **v1.25** registra a **implementação da Fase 32 — Recorrências, Rendas Parceladas & Operações em Grupo** (2026-08-17 — ver `ROADMAP.md` §3 e seção 6 abaixo): todas as entregas concluídas — motor puro `domain/recurrences` (+ testes), migration `0013` (tabelas `recurrences`/`recurrence_skips`, colunas em `expenses`/`incomes`, parcelamento de rendas), 8 RPCs transacionais, estado otimista padrão F30, wizard com bloco de recorrência + parcelas de renda, diálogos com escopo de grupo, seção "Recorrentes" no extrato, insights sem duplicidade e docs atualizadas (`ESPECIFICACAO_TECNICA.md` §3.1/§3.2.5/§3.2.7, `PROJECT_STRUCTURE.md`, `ARCHITECTURE.md`). Suíte **1202 testes / 145 arquivos** (100% verde). **v1.24** registra a **proposta da Fase 32 — Recorrências, Rendas Parceladas & Operações em Grupo** (2026-08-17 — ver `ROADMAP.md` §3 e seção 6 abaixo): cadastro formal de recorrências (template + materialização sob demanda, 4 frequências, fim por data ou nº de ocorrências), parcelamento de rendas (1–60, invariantes D12) e edição/exclusão em grupo (`single | all | subsequent`) para parcelas e recorrências de despesas e rendas. **v1.23** registra a inclusão da **Fase 30 — Importação e Reconciliação Inteligente de Faturas de Cartão (F30)** (ver `ROADMAP.md` §3 e `STATEMENT_RECONCILIATION_PLAN.md`): importação multi-formato (CSV Latin-1/UTF-8, Excel sob demanda, OFX SGML/XML nativo e Quick-Paste), motor heurístico adaptativo sem cabeçalho, deduplicação ordinal SHA-256, auto-atribuição preditiva de categorias e gravação transacional atômica via RPC. **v1.22** registra o **hotfix do Seletor de Pesos — Select dentro de modais** (2026-08-16 — ver `ROADMAP.md` §3, F25): o dropdown do peso no relatório renderizava sem responder a cliques nos modais de edição (wizard funcionava) porque o Dialog modal aplica `pointer-events: none` no body e o wrapper fixed do popper herda esse valor sem estilo próprio (frágil em WebKit); corrigido com `position="popper"` no Select (`sideOffset`/`align`, `min-w` do trigger, `max-h-72`) e regra `[data-radix-popper-content-wrapper] { pointer-events: auto }` em `globals.css` — validado em Chrome headless + CDP (mouse/touch) com o fluxo completo de edição e +2 testes no `expense-detail-dialog.test.tsx`. **v1.24** registra a **refatoração do motor de sugestões preditivas do wizard** (2026-08-16 — ver `ROADMAP.md` §3, F21): Etapa 1 com **ranking temporal ponderado** (janela de ±5–10 dias do mês, relevância temporal × frequência × recência) e **limite estrito de 3 sugestões** (era 5); Etapa 2 com **chips de descrição pura** que atualizam **somente `description`** (bug de sobrescrita do `amount` corrigido) e **filtram nomes de categoria**; código morto removido (`PredictionSuggestions`). **v1.25** registra as **Atualizações Otimistas em Edição/Exclusão de lançamentos** (2026-08-16 — ver `ROADMAP.md` §3): padrão `onMutate`/`onError`/`onSettled` nos hooks de despesa/receita/pagamento de fatura — exclusão e edição refletem **na hora** no Extrato, faturas e relatórios (totais/KPIs recalculam do cache), com **rollback seguro + toast** em falha (novo bus de toasts `services/toast.ts` + `ToastHost`) e sincronização silenciosa em `onSettled`; motor puro `domain/expenses` (`resolveExpenseDeleteIds` — single/all/subsequent). **v1.26** registra a **correção do conflito do Swipe Navigation com os edge swipes nativos do sistema** (2026-08-16 — ver `ROADMAP.md` §3, F20): zona de segurança de borda (`EDGE_INSET_PX` 24px + `isEdgeZoneTouch` — toques iniciados nas extremidades ficam com o gesto de voltar do SO) e **arming com dominância horizontal `|dx| > 1.5·|dy|`** (`isHorizontalDominant` — rolagem vertical com leve drift é descartada no início; cone ±30° mantido na decisão final). **v1.27** registra a **correção do Pull-up to Top instável** (2026-08-16 — ver `ROADMAP.md` §3, F26): **engajamento/âncora no meio do gesto** (atingiu o rodapé durante o `pointermove` → `startY` re-registrado naquele instante), **fim de scroll com `Math.ceil` + tolerância 8px** (subpixel de DPI alto), **`overscroll-behavior-y: contain` + listener `touchmove` não-passivo** (anexado no `pointerdown`, removido no fim — `preventDefault()` só puxando além do rodapé) e **threshold 80 → 60px**. Suíte **1133 testes / 143 arquivos**. **v1.28** registra a **correção da exportação do Fechamento e períodos flexíveis** (2026-08-16 — ver `ROADMAP.md` §3, F22): o documento imprimia **apenas a 1ª página** com lançamentos incompletos (o Chrome corta folhas `position: absolute` na impressão) — corrigido com o primitivo **`PrintSheet`** (portal em nível `body`, fluxo normal de documento; `@media print` esconde o app por `display: none`), validado em Chrome real com **9 páginas / 186 lançamentos completos**; o fechamento agora aceita **mês, ano e período personalizado** via motor `buildDetailedClose` (período-agnóstico) + `periodLabel` no documento. Suíte **1135 testes / 144 arquivos**. **v1.29** registra a **remoção do swipe de navegação entre meses** (2026-08-16 — ver `ROADMAP.md` §3, F20): `MonthSwiper` removido por instabilidade — as 5 telas de mês voltaram ao `MonthPicker` por botões (acessível e previsível); o swipe de sub-abas (`Tabs swipeable`) e o Swipe-to-Action permanecem. Suíte **1131 testes / 143 arquivos**. **v1.30** registra o **endurecimento dos seletores nos modais + sincronização da competência da fatura com a data no modal de edição** (2026-08-16 — ver `ROADMAP.md` §3, F25): validação real em Chrome headless + CDP (touch/mouse, 390/1280px, modais simples e aninhados) confirmou que o dropdown do Select abre, fica por cima (hit-test) e seleciona — o hotfix v1.22 cobre o Chromium; reforços para WebKit em `globals.css` (`z-index: var(--z-modal)` explícito no wrapper do popper — o Radix injeta via efeito, 1 frame depois — + `pointer-events: auto` nas camadas internas `[data-radix-select-content]`/`[data-radix-popover-content]`/`[data-radix-dropdown-menu-content]`); no modal de editar despesa, a **competência recalcula quando a data muda** (fechamento do cartão) preservando override manual (`competenceTouched`), troca de cartão re-baselineia, campo com `inputMode`/`maxLength` + hint e **validação pt-BR de `AAAA-MM`** com mensagem clara. +3 testes no `expense-detail-dialog.test.tsx`. Suíte **1134 testes / 143 arquivos**. **v1.31** registra a **remoção do gesto de pull-up to top** (2026-08-16 — ver `ROADMAP.md` §3, F26): mesmo após as correções do v1.27, o `usePullUpToTop` continuou instável em dispositivos reais — removidos o hook, o `PullUpToTopIndicator` e o motor puro `domain/gestures/overscroll` (+ testes, 29 testes / 3 arquivos); `page-shell.tsx` volta ao contêiner de scroll simples com rolagem nativa intacta e o `overscroll-behavior-y: contain` global permanece. Suíte **1105 testes / 140 arquivos**. **v1.32** registra a **padronização visual dos PDFs de relatório** (2026-08-16 — ver `ROADMAP.md` §3, F22): o documento do Fechamento herdava o tema ativo do app — nos temas dark/oled os PDFs saíam com **fundos escuros** (`bg-surface`/`bg-muted/40` sem override), **cores despadronizadas** e **fontes variadas** (mono `.num` + display Sora). Corrigido na folha `@media print` do `globals.css` com **redefinição das variáveis de tema para a paleta clara fixa** (`!important` vence temas/accentos customizados; utilitárias compilam para `hsl(var(--…))` em runtime → cobertura total e à prova de utilitárias futuras), **tipografia única sans com números tabulares**, `@page` A4/14mm e quebras de página limpas (`tr { break-inside: avoid }` + `thead` repetido por página). Validado em Chrome real + CDP (media print): cores e fonte **idênticas em light/dark/oled e accent gold** e PDF multi-página íntegro (6 páginas em dataset grande). Suíte **1105 testes / 140 arquivos**. **v1.33** registra a **auditoria de código — remoção de código morto e resíduos** (2026-08-16 — ver `ROADMAP.md` §3, v1.22): com `knip` + verificação por import, removidos componentes sem uso (`InstallmentBadge`, `Textarea`, `Slider`, `Accordion` + dependências `@radix-ui/react-accordion`/`react-slider`), funções sem uso (`signOut`, `recalculateBillCompetences`, `findReservedCategory`, `CATEGORY_ICON_OPTIONS`), 10 barrels de features mortos e o script one-off `analyze-halo.mjs`; re-exports mortos removidos dos barrels `ui`/`modules`/`layout`; `zod` declarado como dependência direta; refatorações de performance (`DebtRow` extraído do `DebtsPage` — componente aninhado remontava as linhas a cada render — e `KpiCard` com API unificada via `cents` + NumberTicker com sinal). Funções de domínio testadas mantidas (convenção AGENTS.md §3). Suíte **1102 testes / 139 arquivos**. **v1.34** registra a **exportação da fatura do cartão em CSV** (2026-08-16 — ver `ROADMAP.md` §3, F22): botão "Exportar fatura" na página de Cartões baixa `fatura_<cartao>_<competencia>.csv` com apenas os gastos do cartão na competência (Data, Descrição, Categoria, Valor, Valor p/ relatório, Parcelas) via novo serializador puro `serializeCardInvoiceCsv` — comparação direta com a fatura do banco. +2 testes. Suíte **1104 testes / 139 arquivos**. **v1.35** registra a **evolução da exportação da fatura do cartão — CSV Excel-safe + versão PDF** (2026-08-16 — ver `ROADMAP.md` §3, F22): o CSV saía com "caracteres e formatação bugadas" no Excel — causa raiz: `formatCsvDecimal` emitia separador de milhar (`1.234,56`), que o Excel interpreta como **texto**; corrigido para número sem agrupamento (`1234,56`) mantendo BOM/`;`/CRLF. Nova versão **PDF**: botão "Imprimir / Salvar PDF" na página de Cartões abre o preview no modal com o componente `CardInvoicePrintView` (resumo bruto/ponderado/pago/saldo + gastos do cartão + pagamentos/estornos, padrão F22 — `print-area` + portal `PrintSheet`, paleta clara fixa e paginação multi-página); "Exportar CSV" renomeado para clareza. +5 testes do documento imprimível. Suíte **1109 testes / 139 arquivos**. **v1.36** registra a **auditoria de fluxo do usuário — correções de usabilidade** (2026-08-16 — ver `ROADMAP.md` §3, v1.25): **(1) Lembretes** passam a incluir dívidas vencidas de meses anteriores (filtro de data removido — o motor decide a janela) e todas as faturas com saldo por cartão; **(2) Metas de renda** com erro inline em falha de rede (sem unhandled rejection); **(3) Novo `ErrorState`** (Alert + Tentar novamente com refetch das queries ativas) em Insights, Relatórios, Orçamentos, Dívidas, Categorias e Lembretes; **(4) Edição otimista** de despesa/receita preserva os dados em falha (modal fica aberto com o erro); **(5) Swipe "Excluir"** no extrato abre direto a confirmação (dois diálogos Radix no mesmo commit ficam aria-hidden — a confirmação renderiza sem o modal de detalhes por trás); **(6)** despesa no crédito exige cartão cadastrado (validação clara); **(7)** quitação com erro de categorias visível + retry; **(8)** exclusão de dívidas/categorias dentro do formulário de edição. Suíte **1116 testes / 139 arquivos** (+7 testes novos). **v1.37** registra a **padronização dos botões de exclusão nos modais de detalhe** (2026-08-16 — ver `ROADMAP.md` §3, v1.26): "Excluir despesa"/"Excluir receita" viraram **apenas "Excluir"** nos rodapés dos modais de detalhe (contexto dado pelo próprio modal); varredura confirmou que nenhum outro modal usa esse padrão de botão-link — os rótulos dos diálogos de confirmação foram mantidos. Testes ajustados. Suíte **1116 testes / 139 arquivos**. **v1.38** registra a **correção da responsividade dos modais no desktop** (2026-08-16 — ver `ROADMAP.md` §3, v1.27): a base `lg:max-w-md` do `Modal` vencia `max-w-*` customizados na cascata (variante `lg:` compilada depois), comprimindo o modal da Fatura e outros no desktop — primitivo ganhou prop **`size`** (`sm`/`md`/`lg`/`xl`) como única classe de largura; callers de Fatura/Fechamento (`xl`) e detalhe de relatório (`lg`) atualizados. Suíte **1116 testes / 139 arquivos**. **v1.39** registra a **separação entre despesas parceladas e à vista na fatura do cartão** (2026-08-16 — ver `ROADMAP.md` §3, F22): as compras **parceladas** (herdadas de meses anteriores) ficam **no topo** e as **à vista** (gastos do mês) **embaixo**, cada grupo ordenado por **data** — o usuário distingue o que é gasto do mês naquela fatura do que veio de meses anteriores. Novo motor puro `splitInvoiceItemsByInstallment` em `domain/cards` (parcela atual > 1 = herdada, incluindo a 1ª de novas compras) aplicado na lista da página de Cartões com cabeçalhos por grupo, contagem e subtítulo "data · parcela x/y"; exportações CSV/PDF mantêm a fatura completa. +5 testes. Suíte **1121 testes / 139 arquivos**. **v1.40** registra o **upgrade do motor de Insights** (2026-08-16 — ver `ROADMAP.md` §3, v1.29): recorrências com agregação mensal real e mediana robusta (`medianOf`), ocorrências com tier de corte/economia estimada/reajuste de preço ≥ 10%/duplicidade no mês; catálogo `KNOWN_SERVICES` expandido (+30 serviços) com matching por tokens (`matchesServiceKey` — chaves curtas exigem token exato, sem falsos positivos); página com aba unificada de Alertas & Diagnósticos. Suíte **1127 testes / 139 arquivos** (+6 testes). **v1.41** registra a **auditoria de fluxo do usuário — resiliência de ações críticas** (2026-08-16 — ver `ROADMAP.md` §3, v1.30): exclusões de dívida/ativo/lançamento com toast de falha e confirmações que fecham em sucesso E em falha; formulário de dívida preserva a edição com erro inline; lembretes com feedback em falha; compartilhar avisa clipboard/não suportado; wizard com confirmação anti-perda ao fechar; Visão Geral com `ErrorState` + retry; metas de renda com indicador de sucesso; busca global exibe erro da query. Suíte **1131 testes / 139 arquivos** (+4 testes). **v1.42** registra a **correção da responsividade de cabeçalhos, barras de ação e rodapés de modais no mobile** (2026-08-16 — ver `ROADMAP.md` §3, v1.31): cabeçalho de despesas da fatura do cartão adaptado para flex-col/sm:flex-row com labels responsivos nos botões de impressão e exportação CSV (`hidden sm:inline`), barra de status da fatura com botões adaptativos, rodapés de modais de impressão/fechamento com empilhamento reverso no mobile e formulário de metas de renda inline. Suíte **1131 testes / 139 arquivos**. **v1.47** registra a **verificação final de erros e fragilidades** (2026-08-16 — ver `ROADMAP.md` §3, v1.36): `useSetFeedback` sem `onError` — falha ao salvar avaliação de insight ficava silenciosa; adicionado toast de erro (padrão das demais mutações). Varreduras limpas (mutations com catch, listeners com cleanup, sem logs/any/ts-ignore). Suíte **1134 testes / 139 arquivos**.
**v1.46** registra a **auditoria de código (3ª passada — resíduos e exports mortos)** (2026-08-16 — ver `ROADMAP.md` §3, v1.35): 26 exports mortos removidos — re-exports de chaves de query no barrel `state`, 10 chaves internas un-exportadas, re-exports mortos de `AuthShell`/`TransactionListPage` nos barrels de features, e constantes/funções internas un-exportadas (`AXIS_LOCK_TANGENT` removida, `FLICK_*`/`ACTIVATION_*`, `KNOWN_SERVICES`/`tierOf`/`isKnownService`/`isSubscriptionCategory`/`hasStableValue`, `HIGH_SPEND_THRESHOLD_PERCENT`/`REDUCE_*`, `ONBOARDING_STEP_IDS`, `PACE_MIN_ELAPSED_PERCENT`, `CSV_DELIMITER`). Falsos positivos preservados (`vitest-axe.d.ts`, edge function `quotes`, exports de domínio com testes). Suíte **1134 testes / 139 arquivos**.
**v1.45** registra a **auditoria de fluxo do usuário (3ª passada) — lembretes navegáveis e feedback de pagamento** (2026-08-16 — ver `ROADMAP.md` §3, v1.34): os cards da Central de Lembretes **não eram clicáveis** (o `link` de deep-link era montado mas nunca usado) — agora são clicáveis e acessíveis (`role=button`, Enter/Espaço) e navegam com o deep-link, com **destaque da dívida específica** (`/dividas?q=<id>`) e do cartão/mês nas faturas (+1 teste); **pagamento/estorno de fatura sem feedback de sucesso** — os hooks agora exibem toast "Pagamento registrado"/"Estorno registrado" ao confirmar. Demais fluxos revalidados sem ação (formulários com anti-duplo-clique e anti-perda, exclusões com rollback/toast, exportação com loading por ação). Suíte **1134 testes / 139 arquivos**.
**v1.44** registra a **auditoria arquitetural da camada de dados — N+1, waterfalls e atomicidade** (2026-08-16 — ver `ROADMAP.md` §3, v1.33): varredura de repositórios, hooks e componentes confirmou a camada saudável (sem fetch em loop, posição da carteira batchada em `Map`, categorias resolvidas por batch, cache TanStack compartilhado entre busca/predição/overview, payloads seletivos e contagens `head: true`). **Correção:** `deleteCardPayment` fazia **2 DELETEs sequenciais sem transação** (renda `[REFUND]` + pagamento) — inconsistência se o 2º falhasse; criado o **RPC transacional `delete_card_payment`** (migração `0011`, valida ownership, remove ambos atômicamente com audit D2) e o repositório passou a delegar ao RPC (+1 teste). Suíte **1133 testes / 139 arquivos**.
**v1.43** registra a **auditoria de código — remoção de resíduos e DRY** (2026-08-16 — ver `ROADMAP.md` §3, v1.32): 74 exports mortos removidos dos barrels (`state`, `domain/export`, `domain/gestures`, `domain/insights`, `domain/money`), funções sem uso deletadas (`useSetManualPrice`/`useRemoveManualPrice`, `lastDayOfMonth`, `subscribeCalculatorTarget`, `sentryDsn`/`downloadBlob`/`subscribe*` un-exported) e script one-off `migrate-legacy-data.mjs` removido; **bug corrigido** no snooze de lembretes (snoozeUntil "NaN-NaN-NaN" — lembrete adiado nunca voltava; agora `addDaysISO(today, 7)` +1 teste) e **DRY** dos helpers de soma de dias no canônico `addDaysISO`. Suíte **1132 testes / 139 arquivos**.
...
> **Status: ✅ concluída (2026-08-15)** — ver progresso detalhado no `ROADMAP.md` §3. Resumo: motor puro `src/domain/gestures/swipe.ts` (axis-lock ±30°, flick, activation, boundary resistance — 20 testes); engine `useSwipeNavigation` (máquina idle→tracking→locked→settled, ignoreSelectors, touch/pen + isPrimary, haptics — 13 testes); `MonthSwiper` nas 5 telas de mês (DRY); `Tabs` com prop `swipeable` nas 6 telas de abas; isolamento com `data-swipe-nav-ignore` nos gráficos com scrub (DailyFlowChart, CategoryDonut). **Remoção (2026-08-16):** o `MonthSwiper` (swipe entre meses) foi **removido** por instabilidade — as 5 telas de mês voltaram ao `MonthPicker` por botões; o swipe de sub-abas (`Tabs swipeable`) e o Swipe-to-Action permanecem.
...
### Fase 26 — Gesto Interativo de Retorno ao Topo (Pull-up Overscroll UX) (Trilha A) — ✅ Concluída (2026-08-15) · ❌ Removida (2026-08-16)

> **Origem:** diagnóstico arquitetural (2026-08-15) — ver §1.6. Substituição do botão lateral flutuante legado (`ScrollToTopButton`) por gesto de overscroll vertical no final da rolagem, garantindo zero interferência visual na interface móvel e eliminação de disparos involuntários. **Status:** implementada, verificada e **removida em 2026-08-16** — o gesto continuou instável em dispositivos reais mesmo após as correções de confiabilidade; a rolagem nativa do contêiner `main` segue intacta (ver `ROADMAP.md` §3, F26).
...
F26 (Pull-up Overscroll to Top)            ← Trilha A / Gesture UX (✅ Concluída · ❌ removida 2026-08-16)
...
F30 (Importação de Faturas)                ← Trilha A / Cartões & Inteligência (📋 Pronta para Execução)
F31 (Micro-Interações Obsidian Glass)      ← Trilha A / UI & Micro-Interações (✅ Concluída)
F32 (Recorrências, Rendas Parceladas)      ← Trilha A / Transações (✅ Implementada 2026-08-17 — ver `ROADMAP.md` §3)
```

**Pré-requisitos transversais:** suíte 1134 testes verde + typecheck/lint/build (CI `ci.yml`) · deploy F1.7 (edge function + cron) se P1 = (a) · sem migrations obrigatórias fora de P4 = (c).

**Referências:** `docs/ROADMAP.md` (F0–F13, F14–F26, F30–F32) · `docs/ARCHITECTURE.md` (camadas) · `docs/PROJECT_STRUCTURE.md` (onde criar) · `docs/DESIGN_SYSTEM.md` (visual) · `docs/RELEASE.md` (QA/corte).

---

## 6. FASE 32 — RECORRÊNCIAS, RENDAS PARCELADAS & OPERAÇÕES EM GRUPO (✅ IMPLEMENTADA 2026-08-17)

> **✅ Implementada (2026-08-17)** — todas as entregas concluídas (ver `ROADMAP.md` §3, Fase 32): motor `domain/recurrences` + migration `0013` + 8 RPCs + estado otimista + wizard/diálogos/extrato/insights + docs (`ESPECIFICACAO_TECNICA.md` §3.1/§3.2.5/§3.2.7, `PROJECT_STRUCTURE.md`, `ARCHITECTURE.md`). Suíte **1202 testes / 145 arquivos** (100% verde).

**Objetivo:** formalizar lançamentos recorrentes (despesas e rendas), habilitar o parcelamento de rendas e estender a edição/exclusão em massa (`single | all | subsequent`) a parcelas e recorrências de despesas e rendas.

### Decisões de produto (fechadas em 2026-08-17)

| Decisão | Escolha | Justificativa |
|---|---|---|
| Modelo de recorrência | Template (`recurrences`) + materialização sob demanda | Recorrência sem gerar linhas em massa; fonte da verdade única; ocorrências materializadas por mês via RPC idempotente (D12: cliente calcula, servidor valida) |
| "Em massa" | Operações de grupo (single/all/subsequent) na edição e exclusão | Estende o padrão já existente de exclusão de parcelas (D1) |
| Frequências | Mensal + semanal + trimestral + anual | Enum extensível; cobertura comum de recorrências |
| Horizonte | Fim sempre definido (`end_date` XOR `occurrences_total`, CHECK) | Recorrência finita e previsível; sem geração ilimitada |
| Valor em grupo | Parcelas: valor individual (RPC mantém `value`+`base_amount`); Recorrências: valor em grupo | Preserva o invariante de soma (spec §3.2.2) e a auditoria de pesos (`base_amount` no recebimento integrado); recorrência não tem invariante de soma |

### Escopo de dados (migration 0013)

- `recurrences`: `kind` (`expense|income`), `frequency` (`monthly|weekly|quarterly|yearly`), `value > 0`, `category_id`, `start_date`, `end_date` XOR `occurrences_total`, `report_weight`, `is_active`, campos por tipo (`payment_method`/`card_id` p/ despesa; `receive_type` p/ renda).
- `recurrence_skips`: `(recurrence_id, occurrence_date)` — excluir apenas uma ocorrência não regenera.
- `expenses` += `recurrence_id`, `occurrence_number` (unique `(recurrence_id, date)` para idempotência).
- `incomes` += `installments_total` (1–60), `installment_number`, `installment_group_id`, `recurrence_id`, `occurrence_number` + CHECKs (grupo presente sse parcelado; avulso XOR parcela XOR recorrência).

### RPCs (D12: cliente calcula, servidor valida invariantes)

`create_recurrence` · `materialize_recurrences(p_items)` (uma chamada por mês, idempotente, respeita skips) · `delete_recurrence_occurrences` (`single` → linha + skip; `all` → template + linhas + dívidas pendentes; `subsequent` → trunca template + linhas a partir da ocorrência) · `update_recurrence_occurrences` · `create_income_installments` (espelho do `create_expense_with_debt`) · `delete_income_installments` · `update_income_installments_group` · `update_expense_installments_group`.

> **Consolidação na execução:** os 2 RPCs individuais previstos (`update_expense_installment`/`update_income_installment`) foram **absorvidos pelo modo `single`** dos RPCs de grupo — a edição individual de parcela passa pelo mesmo caminho transacional e mantém `value` + `base_amount` consistentes (DRY, 8 RPCs no total).

### UI

- Wizard: parcelas para renda; bloco "Recorrência" (frequência + fim) com prévia das ocorrências na revisão.
- Extrato: seção "Recorrentes" com subtítulo `x/N`.
- Diálogos de detalhe: seletor de escopo (single/all/subsequent) na edição e exclusão; valor desabilitado em grupo para parcelas (hint "editar o total = excluir e refazer").
- Insights: recorrências formais (`recurrence_id`) excluídas da detecção histórica.

**Arquivos:** `src/domain/recurrences/*` (+ testes) · `supabase/migrations/20260101000013_recurrences.sql` · `src/data/rpc.ts` · `src/types/schema.ts` · `src/types/database.ts` · `src/state/` (queries + mutations) · `src/features/transactions/` (wizard, diálogos, extrato) · `src/domain/insights/recurrences.ts`.

**✅ DoD:** lançamento de despesa recorrente (4 frequências, fim por data ou nº); renda parcelada (1–60, invariantes) e recorrente; edição/exclusão em grupo nos 3 modos para ambos; materialização idempotente e lazy; edição individual de parcela preserva soma/`base_amount`; insights sem duplicidade; testes de domínio/RPC/UI + typecheck/lint/build; docs atualizadas.
