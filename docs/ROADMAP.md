...
> **Status:** v1.8 — **plano de execução canônico** do projeto (o `ESPECIFICACAO_TECNICA.md` §6 o referencia como resumo executivo). Foco em **ordem de execução**, **ordem de construção da UI (Design System primeiro)** e **Definition of Done (DoD)** por fase. **v1.48** registra a **padronização arquitetural do ciclo de vida dos modais de formulário / edição (Modal Content with Key Pattern)** (2026-08-16): correção da fragilidade em que modais de edição abriam com dados vazios (`CategoryFormDialog`, `DebtFormDialog`, `LimitDialog`, `AssetFormDialog`, `TransactionFormDialog`, `SettleDialog`, `PaymentDialog` e `TransactionListDialog`) — a prop `open={true}` não dispara o evento `onOpenChange` do Radix Dialog, mantendo estados vazios do mount inicial da página; formulários agora são isolados em subcomponentes montados condicionalmente sob demanda com `key={item?.id ?? 'new'}` dentro de `{open ? <Content ... /> : null}`, garantindo inicialização 100% fiel de dados, limpeza automática no unmount e ação de novo lançamento corrigida no extrato de ativos da carteira. Suíte **1139 testes / 141 arquivos** (+3 testes novos).
