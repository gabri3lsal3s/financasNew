...
> **Status:** v1.8 — **plano de execução canônico** do projeto (o `ESPECIFICACAO_TECNICA.md` §6 o referencia como resumo executivo). Foco em **ordem de execução**, **ordem de construção da UI (Design System primeiro)** e **Definition of Done (DoD)** por fase. **v1.53** registra o **refinamento estético definitivo do PWA — Splash Screen reativo, ícones transparentes e sincronização dinâmica da barra de título** (2026-08-17): **(1) Splash Screen 100% reativo ao tema do usuário**: bootstrap inline em `index.html` lê `localStorage` imediatamente antes do render, aplicando `data-theme` no `<html>`, `<body>` e `#app-splash` (Light `#F4F7F9`, Dark `#0C1923`, OLED `#000000`) sem nenhum flash branco; **(2) Ícones 100% transparentes e remoção de caixa branca**: pipeline em `generate-icons.mjs` atualizado para renderizar todos os ícones maskable e PWA (`maskable-192.png`, `maskable-512.png`, etc.) com canal alpha transparente puro (`0,0,0,0`), eliminando o fundo branco em todos os launchers e splash screens do SO; **(3) Zero texto e alinhamento milimétrico**: splash screen simplificado em flexbox perfeitamente centralizado com ícone 104px, halo radial e barra de progresso proporcional, sem nenhum elemento textual; **(4) Barra de título sincronizada com o tema**: sincronização contínua de `<meta name="theme-color">` via bootstrap (`index.html`) e reativamente no `ThemeProvider` (`theme-provider.tsx`), garantindo que a barra de título do SO/janela PWA e a status bar mobile assumam a cor exata do tema escolhido; utilitários `.app-region-drag` e `.app-region-no-drag` aplicados no cabeçalho do `PageShell`. Suíte **1159 testes / 142 arquivos** (100% verde).
