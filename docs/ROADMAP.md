...
- [x] **Header alinhado à página:** conteúdo centralizado num container `max-w-5xl` (mesmos limites do conteúdo); no desktop a **busca vira barra inline responsiva** (`GlobalSearch` com `lg:flex-1`, um único elemento: ícone no mobile, barra com dica Ctrl+K no desktop) tomando a largura excedente entre o limite esquerdo da página e os botões, que ficam alinhados ao limite direito da página.
