...
> **v1.99** registra o **Suporte Pleno ao Teclado Físico na Calculadora Flutuante & Atalho Global `F9`** (2026-08-24): **(1) Atalho Global `F9`**: registro de listener global para abrir/fechar a calculadora flutuante instantaneamente em qualquer tela do app via tecla de função `F9`; **(2) Digitação e Operações Completas pelo Teclado**: suporte completo a digitação numérica (`0`–`9`), separadores decimais (`,` e `.`), operadores aritméticos (`+`, `-`, `*` / `x` / `X`, `/`), avaliação de expressões (`Enter` e `=`), remoção de dígitos (`Backspace`) e reset/limpeza (`c`, `C`, `Delete`); **(3) Injeção Contextual Rápida com `Ctrl+Enter` / `Cmd+Enter`**: atalho de teclado direto para acionar a injeção do valor calculado no campo monetário ativo em foco e fechar a calculadora; **(4) Suíte 100% Verde**: 218 arquivos / 1.626 testes passando, zero erros de typecheck e lint.
