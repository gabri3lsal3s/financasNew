...
- **Problema corrigido:** ao abrir modais e drawers (bottom sheet) no mobile, o teclado virtual era acionado automaticamente antes de qualquer interação do usuário, bloqueando metade do viewport. Causa raiz dupla: (1) `@radix-ui/react-dialog` foca o primeiro elemento interativo por padrão ao abrir o `DialogContent`; (2) múltiplos componentes de formulário usavam `autoFocus` explicitamente em campos `Input` e `MoneyInput`.
...
- **Remoção de `autoFocus` em campos de texto/número (10 arquivos):** `limit-dialog.tsx`, `card-form-dialog.tsx`, `payment-dialog.tsx`, `category-form-dialog.tsx`, `debt-form-dialog.tsx`, `early-amortization-dialog.tsx`, `loan-form-dialog.tsx`, `settle-dialog.tsx`, `asset-form-dialog.tsx` e `step-value.tsx` (Wizard passo 1). Apenas campos `Input` e `MoneyInput` foram tratados — os únicos que disparam o teclado virtual no mobile.
- **Regra de distinção `<button>` vs `<input>`:** `autoFocus` em `<button>` **nunca abre o teclado virtual** e é permitido quando serve a um propósito funcional de segurança. O botão **Cancelar** do `ConfirmDialog` mantém `autoFocus`: garante que ao abrir um diálogo destrutivo (ex.: "Excluir cartão?") pressionar `Enter` imediato acione Cancelar — ação segura — e não o botão de confirmação. Compatível com o `onOpenAutoFocus={(e) => e.preventDefault()}` do Radix: o browser processa o `autoFocus` nativo do botão antes de o Radix tentar gerenciar o foco.
- **Sem regressão de acessibilidade:** teclado físico navega por Tab normalmente; Esc e clique no backdrop fecham sem falhas; o teclado virtual só abre quando o usuário toca deliberadamente num campo de entrada.
