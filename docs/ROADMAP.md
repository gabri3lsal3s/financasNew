...
> **v2.26** registra o **Saneamento Atômico do Marco Zero, Migração 0043 e Integridade Matemática da TIR (XIRR)** (2026-09-04):
> - **(1) Diagnóstico de Integridade da TIR (XIRR)**: Identificada e corrigida a inconsistência que gerava TIR negativa (-33,9% a.a.) apesar do patrimônio apresentar ganho real positivo (+19,1%). A causa raiz era o acúmulo de múltiplos registros de Marco Zero duplicados na tabela `portfolio_contributions`, que somavam um fluxo de saída de capital de R$ 65.385,98 frente a apenas R$ 18.660,01 de patrimônio vivo;
> - **(2) Migration SQL 0043 (`0043_fix_marco_zero_duplicates.sql`) & RPC Atômico `upsert_marco_zero`**: Limpeza por `PARTITION BY user_id` mantendo estritamente o registro mais recente, e implementação da função RPC Postgres transacional `upsert_marco_zero` que remove atomicamente qualquer Marco Zero anterior do usuário e insere o novo calibrado em uma única transação no servidor (garantia de no máximo 1 Marco Zero por usuário);
> - **(3) Camada de Estado & UI Refatorada**: Criação do hook `useUpsertMarcoZero`, eliminação da exclusão sequencial no cliente em `InitialPocketCostDialog`, resolução de seleção do registro mais recente com `created_at` e correção de compatibilidade de variantes do Design System (`Badge` e `DatePicker`);
> - **(4) Suíte 100% Verde e Build Limpo**: 280 arquivos de teste / 2.014 testes passando (100% verde), zero erros de typecheck (`tsc -b`), lint sem emojis e build de produção Vite concluído com sucesso em 2.67s.
