...
> **Status:** v1.8 — **plano de execução canônico** do projeto (o `ESPECIFICACAO_TECNICA.md` §6 o referencia como resumo executivo). Foco em **ordem de execução**, **ordem de construção da UI (Design System primeiro)** e **Definition of Done (DoD)** por fase. **v1.65** registra a **Conclusão da Fase 34 — Importação e Reconciliação Inteligente de Extratos Bancários & Transações Gerais (Conta Corrente / PIX / TED)** (2026-08-18): **(1) Motor Puro de Domínio**: módulo `src/domain/reconciliation/bank-reconciliation.ts` com `reconcileBankTransactions`, isolando despesas (`payment_method != 'credit_card'`) e receitas (`incomes`), com scoring multidimensional, predição de categorias e detecção de sobras no app (`unmatchedExistingExpenses` e `unmatchedExistingIncomes`); **(2) Blindagem contra as 6 Fragilidades Financeiras**: travas anti double-counting (`isCreditCardPayment`), anti inflação (`isInternalTransferOrInvestment`), detecção textual de entradas (`isLikelyIncomeDescription`), reconhecimento de estornos (`isRefundOrReturn`) e suporte a colunas indicadoras `D/C`; **(3) Infraestrutura de Banco & RPC Transacional**: migração `20260101000017_bank_statement_import.sql` com índice único condicional de idempotência `idx_incomes_user_statement_hash` e RPC `import_bank_transactions` com inserção atômica de despesas e receitas e auditoria em `audit_events`; **(4) Estado & TanStack Query**: hook `useImportBankTransactions` com invalidação atômica e coordenada de despesas, receitas, orçamentos e visão geral; **(5) Interface de Reconciliação em 3 Passos**: `BankStatementImportDialog` integrado em `TransactionListPage` com abas de upload (OFX/CSV) e Quick-Paste (linguagem natural), ajuste de mapeamento e tabela de conferência com filtros rápidos e badges. Suíte **1.317 testes / 165 arquivos** (100% verde).
