...
- [x] **FAB da calculadora contextual:** o botão flutuante deixou de aparecer nas páginas comuns — fica **apenas o ícone no header** (`CalculatorButton`). O FAB só surge **quando há um campo de valor ativo** (MoneyInput focado — modais de formulário e wizard), usando a assinatura do `calculator-bridge` (`subscribeCalculatorTarget`) para reagir ao foco; sem campo ativo, o FAB não renderiza.
- [x] **Bugfix — FAB persistia após fechar o modal:** o `MoneyInput` registrava o campo no focus mas nunca desregistrava; ao fechar o modal (campo desmonta), o `activeTarget` permanecia e o FAB ficava visível na página. Corrigido com cleanup no unmount (`calculatorTargetRef` + `unregisterCalculatorTarget` no efeito) — **sem** desregistrar no blur, para preservar o fluxo "Usar valor" (o foco sai do campo quando o painel da calculadora abre). + teste dedicado.
