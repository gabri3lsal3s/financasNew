...
> **v1.96** registra a **Auditoria, Refatoração Estrutural, Responsividade Mobile e Padronização de PDF do Módulo de Relatórios** (2026-08-24): **(1) Alinhamento do Contêiner com `PageShell`**: remoção de espaçamento duplo e paddings redundantes (`p-3.5 sm:p-6 pb-20`) na `ReportsPage` e nos estados de Skeleton e ErrorState, adotando `<div className="flex flex-col gap-6 w-full min-w-0">` com alinhamento pixel-perfect; **(2) Desduplicação e Unificação dos Modais (`ReportDocumentLayout` & `Modal`)**: adição da prop `headerActions?: ReactNode` ao cabeçalho nativo do primitivo `Modal`, integrando o botão "Imprimir / Salvar PDF" e ações contextuais diretamente no cabeçalho Radix Dialog, eliminando cabeçalhos e botões "X" de fechar duplicados em todos os 5 modais de dossiê; **(3) Responsividade de Tabelas (`ReportTable` & `DataList`)**: larguras protegidas e alinhamento tabular (`w-24 sm:w-28 shrink-0`) nas colunas "Valor Bruto", "Ponderado" e "Part. %", prevenindo esmagamento em telas `< 380px`; **(4) Engine de Impressão e PDF A4**: regras de `@media print` no `globals.css` garantindo `print:overflow-visible` e `width: 100%`, classes `break-inside-avoid` em todas as seções/gráficos e larguras percentuais fixas na tabela de 8 colunas de Custódia em `WealthTearSheetModal` evitando cortes na margem direita em A4 Portrait; **(5) Fuso Horário Local e Formatação**: adoção de `todayISO()` em `excel-export-card.tsx` e `<MoneyText>` em todas as métricas dos modais; **(6) Suíte 100% Verde**: 215 arquivos / 1.602 testes passando, zero erros de lint e typecheck.
