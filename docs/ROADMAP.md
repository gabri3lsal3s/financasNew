...
> **Status:** v1.7 — **plano de execução canônico** do projeto (o `ESPECIFICACAO_TECNICA.md` §6 o referencia como resumo executivo). Foco em **ordem de execução**, **ordem de construção da UI (Design System primeiro)** e **Definition of Done (DoD)** por fase. **v1.2** inseriu formalmente as fases **F14–F18** (Trilha A — UI/UX prioritária · Trilha B — carteira de investimentos completa); **v1.3** adicionou a **F19 — Inteligência & Consistência dos Insights** (Trilha A); **v1.4** adicionou a **F20 — Sistema de Gestos & Navegação por Swipe (Mobile Gesture UX)** (Trilha A); **v1.5** inseriu as fases **F21–F24** (Inteligência de Entrada, Exportação/Fechamento, Engenharia de Performance e Planejamento FIRE); **v1.6** formalizou a **F25 — Micro-interações, Feedback Visual & Ergonomia de Interface (Desktop & Mobile)**; **v1.7** formalizou a **F26 — Gesto Interativo de Retorno ao Topo (Pull-up Overscroll UX)**; **v1.8** registra a **conclusão da F27 — Insights: Precisão, Deduplicação & Casos de Borda** (2026-08-15); e **v1.9** registra a **conclusão da F28 — Investimentos: Mobile Responsive & Organização** (2026-08-15); **v1.10** registra a **conclusão da F29 — Insights: Inteligência de Recorrências, Alertas & Diagnósticos unificados e Polish do Planejamento** (2026-08-15); **v1.11** registra o **hotfix de responsividade do DatePicker** (2026-08-16 — ver §3, F25); **v1.12** registra o **hotfix do Seletor de Pesos (Select dentro de modais)** (2026-08-16 — ver §3, F25); **v1.13** registra a **refatoração do motor de sugestões preditivas do Wizard** (2026-08-16 — ver §3, F21): ranking temporal ±5–10 dias com limite estrito de 3 sugestões na Etapa 1 e chips de descrição pura na Etapa 2 sem sobrescrever o valor preenchido; **v1.14** registra as **Atualizações Otimistas em Edição/Exclusão de lançamentos** (2026-08-16 — ver §3): Extrato, faturas de cartão e relatórios refletem edições e exclusões na hora (cache React Query em `onMutate`), com rollback seguro + toast em `onError` e sincronização em `onSettled`; **v1.15** registra a **correção do conflito do Swipe Navigation com os edge swipes nativos do sistema** (2026-08-16 — ver §3, F20): zona de segurança de borda (edge inset 24px) e arming com dominância horizontal `|dx| > 1.5·|dy|`; **v1.16** registra a **correção do Pull-up to Top instável** (2026-08-16 — ver §3, F26): engajamento/âncora no meio do gesto, detecção de fim com `Math.ceil` + tolerância 8px, listener `touchmove` não-passivo e `overscroll-behavior-y: contain`. **v1.17** registra a **correção e evolução do Fechamento/Exportação** (2026-08-16 — ver §3, F22): o documento de fechamento imprimia **apenas a primeira página** (o Chrome cortava o `position: absolute` da folha de impressão na 1ª página) — resolvido com o novo primitivo `PrintSheet` (portal em nível `body` com fluxo normal + `@media print` escondendo o app por `display: none`), validado em Chrome headless + CDP com **9 páginas / 186 lançamentos completos**; e o fechamento passou a aceitar **mês, ano e período personalizado** (motor renomeado para `buildDetailedClose`, período-agnóstico). **v1.18** registra a **remoção do swipe de navegação entre meses** (2026-08-16 — ver §3, F20): o gesto horizontal do `MonthSwiper` foi **removido** por instabilidade — as 5 telas de mês voltaram ao `MonthPicker` por botões (acessível e previsível); o swipe de sub-abas (`Tabs swipeable`) e o Swipe-to-Action de despesas permanecem. **v1.19** registra o **endurecimento dos seletores nos modais + sincronização da competência com a data** (2026-08-16 — ver §3, F25): validação real em Chrome headless + CDP (touch/mouse, 390/1280px, modais simples e aninhados) confirmou que o dropdown do Select abre/seleciona no Chromium; reforços para WebKit em `globals.css` — `z-index: var(--z-modal)` explícito no wrapper do popper + `pointer-events: auto` nas camadas internas (`[data-radix-select-content]`, `[data-radix-popover-content]`, `[data-radix-dropdown-menu-content]`); no modal de editar despesa, a **competência da fatura recalcula automaticamente quando a data muda** (fechamento do cartão) preservando override manual (`competenceTouched`), troca de cartão re-baselineia, campo com `inputMode`/`maxLength` + hint, e **validação pt-BR de `AAAA-MM`** no submit com mensagem clara (antes, formato inválido falhava no servidor com erro genérico). +3 testes. **v1.20** registra a **remoção do gesto de pull-up to top** (2026-08-16 — ver §3, F26): o `usePullUpToTop` continuou instável em dispositivos reais mesmo após as correções — removidos o hook, o `PullUpToTopIndicator` e o motor puro `domain/gestures/overscroll` (+ testes); `page-shell.tsx` volta ao contêiner de scroll simples e a rolagem nativa fica intacta. **v1.21** registra a **padronização visual dos PDFs de relatório** (2026-08-16 — ver §3, F22): o documento do Fechamento **herdava o tema ativo do app** — nos temas dark/oled os PDFs saíam com fundos escuros, cores despadronizadas e fontes variadas (mono + display). Corrigido na folha `@media print` do `globals.css`: **redefinição das variáveis de tema para a paleta clara fixa** (fundo branco, texto quase-preto `#142531`, bordas cinza-claras, acentos da identidade — `!important` para vencer temas/accentos customizados), **tipografia única sans com números tabulares**, `@page` A4/14mm e `tr { break-inside: avoid }` + `thead` repetido por página. Validado em Chrome real + CDP com media print: cores e fonte **idênticas em light/dark/oled e accent gold** e PDF multi-página íntegro (6 páginas em dataset grande). **v1.22** registra a **auditoria de código — remoção de código morto e resíduos** (2026-08-16): com `knip` + verificação por import, removidos **componentes sem uso** (`InstallmentBadge`, `Textarea`, `Slider`, `Accordion` + teste — e as dependências `@radix-ui/react-accordion`/`react-slider`), **funções sem uso** (`signOut` em `data/auth`, `recalculateBillCompetences` em `data/rpc`, `findReservedCategory` em `categories`, `CATEGORY_ICON_OPTIONS`), **10 barrels de features mortos** (rotas usam lazy deep-path) e o script one-off `analyze-halo.mjs`; **re-exports mortos removidos** dos barrels `ui`/`modules`/`layout`; `zod` declarado como dependência (era transitiva); **refatorações**: `DebtRow` extraído para o nível de módulo no `DebtsPage` (componente aninhado remontava as linhas a cada render) e `KpiCard` com **API unificada** (`valueCents` + `cents` redundantes → só `cents` com NumberTicker e sinal `−` prefixado — `formatCentsAsBRL` zera negativos). Mantidas as funções de domínio testadas (convenção AGENTS.md §3 — API pública com testes). Suíte **1102 testes / 139 arquivos**. **v1.23** registra a **exportação da fatura do cartão em CSV** (2026-08-16 — ver §3, F22): botão **"Exportar fatura"** no cabeçalho da seção de despesas da página de Cartões — baixa `fatura_<cartao>_<competencia>.csv` com **APENAS os gastos lançados no cartão** da fatura selecionada (sem colunas redundantes de cartão/forma; valores REAIS em centavos com conversão na borda). Novo serializador puro `serializeCardInvoiceCsv` + tipo `ExportCardInvoiceRow` em `domain/export` (+2 testes, BOM/`;`/CRLF pt-BR). Suíte **1104 testes / 139 arquivos**. **v1.24** registra a **evolução da exportação da fatura do cartão — CSV Excel-safe + versão PDF** (2026-08-16 — ver §3, F22): o CSV da fatura saía **"desorganizado, com caracteres e formatação bugadas"** no Excel — causa raiz: `formatCsvDecimal` emitia **separador de milhar** (`1.234,56`), que várias versões do Excel interpretam como **texto** (não soma, células desalinhadas); corrigido para **número sem agrupamento** (`1234,56`, vírgula decimal pt-BR) com BOM/`;`/CRLF mantidos (+ testes). Nova **versão PDF da fatura**: botão **"Imprimir / Salvar PDF"** ao lado do "Exportar CSV" abre o preview no modal com o componente **`CardInvoicePrintView`** (`components/modules`) — documento executivo da competência com resumo (bruto/ponderado/pago/saldo aberto, com ponderado quando difere), **apenas os gastos lançados no cartão** (data, descrição, categoria, valor, valor p/ relatório, parcelas) e seção de pagamentos/estornos; segue o padrão F22 (`print-area` + portal `PrintSheet` → paleta clara fixa, tipografia única e paginação multi-página em qualquer tema). +5 testes do documento. Suíte **1109 testes / 139 arquivos**. **v1.25** registra a **auditoria de fluxo do usuário — correções de usabilidade** (2026-08-16): análise funcionalidade por funcionalidade com correções em 8 frentes — **(1) Lembretes**: dívidas vencidas de meses anteriores voltam a aparecer (o filtro `due_date >= 1º dia do mês` descartava atrasadas; agora o motor `debtReminder` decide a janela) e **todas** as competências de fatura com saldo aberto são lembradas (antes só a mais recente via `autoSelectBillMonth` — faturas vencidas antigas ficavam invisíveis); **(2) Metas de renda**: salvamento/remoção com `try/catch` + mensagem inline (antes promise rejeitada não tratada — falha silenciosa); **(3) Novo primitivo `ErrorState`** (Alert + "Tentar novamente" com refetch das queries ativas — AGENTS.md §5) aplicado em Insights, Relatórios, Orçamentos, Dívidas, Categorias e Lembretes (antes só Transações/Cartões/Investimentos tinham retry); **(4) Edição otimista de despesa/receita**: em falha o modal **permanece aberto** com o erro e os dados preservados (antes fechava na hora e perdia o formulário); **(5) Swipe "Excluir"** no Extrato agora abre **direto a confirmação de exclusão** (com modos p/ parcelas; antes abria o modal de detalhes — affordance enganosa). Detalhe técnico: dois diálogos Radix montados no **mesmo commit** ficam ambos `aria-hidden` — a confirmação renderiza sozinha (sem o modal de detalhes por trás); **(6) Despesa no crédito sem cartão cadastrado**: validação com mensagem clara (antes salvava `card_id: null` silencioso); **(7) Quitação**: erro da query de categorias visível com retry (antes Select vazio sem explicação); **(8) Exclusão de dívidas/categorias movida para dentro do formulário de edição** (botão Excluir + confirmação, consistente com os demais modais). Suíte **1116 testes / 139 arquivos** (+7 testes novos; testes de dívidas/categorias adaptados ao novo fluxo de exclusão). **v1.26** registra a **padronização dos botões de exclusão nos modais de detalhe** (2026-08-16): o botão de texto destrutivo no rodapé dos modais de despesa e receita dizia "Excluir despesa"/"Excluir receita" — simplificado para **apenas "Excluir"** (ícone + texto curto, contexto já dado pelo modal). Varredura do padrão (`text-critical` + link) confirmou que só os dois modais de detalhe o usavam; os `confirmLabel` dos diálogos de confirmação ("Excluir cartão", "Excluir categoria"…) foram mantidos por terem contexto próprio. Testes atualizados. Suíte **1116 testes / 139 arquivos**. **v1.27** registra a **correção da responsividade dos modais no desktop** (2026-08-16): o modal da Fatura (e outros com largura customizada) abria comprimido a 448px no desktop — causa raiz: a classe base `lg:max-w-md` do primitivo `Modal` **vence** o `max-w-*` passado via `className` na cascata do Tailwind (a variante `lg:` é compilada depois da utilitária base, mesma especificidade — confirmado no CSS buildado). Correção no primitivo: nova prop **`size`** (`sm`/`md`/`lg`/`xl` → `lg:max-w-sm/md/lg/3xl`) aplicada como ÚNICA classe de largura (sem conflito de cascata); o `lg:max-w-md` saiu da string base. Callers atualizados: Fatura do cartão e Fechamento do período (`size="xl"` = 48rem) e Diálogo de detalhe de relatório (`size="lg"`). Varredura confirmou que nenhum outro componente usa base `lg:max-w-*` conflitante. Suíte **1116 testes / 139 arquivos**. **v1.28** registra a **separação entre despesas parceladas e à vista na fatura do cartão** (2026-08-16 — ver §3, F22): as compras **parceladas** (herdadas de meses anteriores) ficam **no topo** e as **à vista** (gastos do mês) **embaixo**, cada grupo ordenado por **data** — o usuário distingue de imediato o que é gasto do mês naquela fatura e o que veio de meses anteriores. Novo motor puro `splitInvoiceItemsByInstallment` em `domain/cards` (usa `installment_number`/`installment_total` — parcela atual > 1 é herdada, incluindo a 1ª de novas compras) aplicado na lista da página de Cartões com cabeçalhos por grupo, contagem e subtítulo "data · parcela x/y"; a exportação CSV/PDF continua com a fatura completa (sem separação, para conferência com o banco). +5 testes (2 domínio + 3 página). Suíte **1121 testes / 139 arquivos**. **v1.29** registra o **upgrade do motor de Insights — recorrências mais precisas, catálogo de serviços expandido e alertas diagnósticos** (2026-08-16 — ver §3, F29): `detectRecurrences` passou a **agregar por mês real** e usar **mediana robusta** (`medianOf` — imune a outliers/ordem; faturas variáveis como água `[80, 130, 95]` agora contam como recorrência); ocorrências ganharam **tier de corte** (`CutTier`), **economia mensal estimada se cortada** (`savingsIfCutCents`), **reajuste de preço** detectado (aumento ≥ 10% vs. meses anteriores, `PriceAdjustment`) e **duplicidade no mês** (`duplicateChargesThisMonth`). Catálogo `KNOWN_SERVICES` expandido (+30 serviços: academias — smartfit/bluefit/totalpass/gympass/wellhub, pedágios — semparar/veloe/conectcar/tagitau, imprensa — uol/folha/estadao/globo, IA — openai/claude, streaming — twitch/mubi/telecine…) com **matching por tokens** (`matchesServiceKey`/`tokenizeText` — chaves curtas ≤ 3 chars exigem token exato isolado: 'max' não casa 'maxxi', 'tim' não casa 'estimativa'; chaves longas permitem junção 'Smart Fit'→'smartfit'). Página de Insights reestruturada com **aba unificada de Alertas & Diagnósticos** (reajuste de preço, cobranças duplicadas, economia estimada por corte). Suíte **1127 testes / 139 arquivos** (+6 testes). **v1.30** registra a **auditoria de fluxo do usuário — resiliência de ações críticas** (2026-08-16): varredura ponta a ponta das jornadas (navegação, formulários, CRUD, modais, busca) com correções de fragilidades — **(1) Exclusões sem tratamento de erro**: `useDeleteDebt`, `useDeletePortfolioAsset` e `useDeletePortfolioTransaction` ganharam toast de falha (antes promise rejeitada não tratada); o formulário de dívida mantém a edição aberta com erro inline em falha e as confirmações de exclusão de ativo/lançamento fecham em sucesso E em falha (antes ficavam presas); **(2) Lembretes**: `useSetReminderState` com toast em falha (lido/snooze/restaurar não ficam silenciosos); **(3) Compartilhar** despesa/receita avisa via toast quando o texto cai no clipboard ou o navegador não suporta; **(4) Wizard de lançamento**: fechar com dados preenchidos pede confirmação "Descartar lançamento?" (anti-perda acidental); **(5) Visão Geral**: erro passou a usar `ErrorState` com "Tentar novamente" (era só Alert — único ponto fora do padrão AGENTS.md §5); **(6) Metas de renda**: indicador "Salva" com ícone e timer limpo no unmount; **(7) Busca global**: erro da query exibido em vez de "Nenhum resultado". Suíte **1131 testes / 139 arquivos** (+4 testes). **v1.31** registra a **correção da responsividade de cabeçalhos, barras de ação e rodapés de modais no mobile** (2026-08-16): cabeçalho de despesas da fatura do cartão adaptado para flex-col/sm:flex-row com labels responsivos nos botões de impressão e exportação CSV (`hidden sm:inline`), barra de status da fatura com botões adaptativos, rodapés de modais de impressão/fechamento com empilhamento reverso no mobile e formulário de metas de renda inline. Suíte **1131 testes / 139 arquivos**. **v1.36** registra a **verificação final de erros e fragilidades** (2026-08-16): varredura de mutações sem tratamento de erro encontrou **1 caso silencioso** — `useSetFeedback` (avaliação de insight) não tinha `onError`: falha de rede ao salvar a decisão sumia sem feedback. Adicionado toast "Não foi possível salvar a avaliação" (padrão das demais mutações). Demais varreduras limpas: todas as `mutateAsync` com `await`/catch, `.mutate()` restantes com `onError` no hook, listeners com cleanup (o único sem remove é o init do PWA, intencional), zero `console.error/warn`, zero `any`/`@ts-ignore` (único `@ts-expect-error` é o fallback intencional do Web Audio em navegadores legados). Suíte **1134 testes / 139 arquivos**.
