...
- [x] **DailyFlowChart repaginado (visual premium):** barras empilhadas com **trilha de fundo** (`bg-muted/25`, dias vazios visíveis) e escala global (maior dia do mês), `gap-0.5` e cantos arredondados com `overflow-hidden`; scrub com **coluna ativa** (`bg-muted/60`) + **linha guia vertical** em toda a altura; curva de saldo com **área preenchida** (`fill-portfolio/10` até a linha do zero), **linha de referência do zero** (quando o saldo fica negativo) e **ponto final** (saldo de hoje); área da curva maior (`h-14`). Contrato de teste preservado (polyline da curva, meta tracejada, tooltip).
