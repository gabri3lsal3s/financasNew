...
> - **(3) Redirecionamento Automático na Abertura (`resolveLandingPath`)**: Ao abrir na rota raiz `/`, usuários com acesso restrito (ex.: apenas Investimentos e Relatórios) são direcionados imediatamente para `/investments`, impedindo a montagem da `OverviewPage` e suprimindo queries desnecessárias de extrato, cartões e orçamentos;
> - **(4) Isolamento de Queries na Central de Relatórios (`ReportsPage`)**: Suporte a `{ enabled: hasFinanceFeatures }` em `useExpenses`, `useIncomes`, `useDebts` e `useCategories`, e seleção inteligente da aba inicial em `"investimentos"` para perfis sem o core financeiro;
> - **(5) Catálogo Administrativo de Módulos**: Inclusão explícita de `overview` na lista de `AVAILABLE_MODULES` no `UserEditDialog` e `CreateInviteDialog`;
> - **(6) Suíte 100% Verde & Zero Lints**: 282 arquivos de teste / 2.027 testes passando (100% verde), zero erros de typecheck (`tsc -b`) e lint estrito (`eslint .`).

> **v2.27** registra o **Assistente Inteligente de Extrato, Suporte Nativo a Resgates do Bolso e Modo Rápido Consolidado** (2026-09-07):
> - **(1) Assistente Inteligente de Extrato (`statement-parser.ts`)**: Parser puro no domínio capaz de processar extratos colados de corretoras/bancos (XP, BTG, Itaú, Kinvo, Gorila, Excel) com colunas de Mês/Ano, Valor Aplicado e Saldo. Calcula deltas mensais automáticos, categorizando em Aportes (+), Resgates (-) e meses sem movimentação, com pré-visualização completa e gravação em lote (`useBatchCreateHistoricalContributions`);
> - **(2) Suporte Nativo a Resgates Históricos do Bolso**: Mapeamento completo de fluxos de caixa em `buildPortfolioCashFlows` e `calculateNetInjectedCapital` em `src/domain/portfolio/irr.ts` reconhecendo retiradas históricas (tag `[Resgate]`), computando devolução antecipada de capital ao bolso e abatendo do capital líquido;
> - **(3) Interface Expandida (`InitialPocketCostDialog`, size="xl")**: Painel com 3 cards de métricas (Total Aportado, Total Resgatado, Capital Líquido do Bolso), navegação em 3 abas (`Tabs`: Assistente de Extrato, Lançamento Individual e Modo Rápido Consolidado), seletor nativo Aporte (+) / Resgate (-) e edição inline;
> - **(4) Suíte 100% Verde & Build de Produção Limpo**: 281 arquivos de teste / 2.023 testes passando (100% verde), zero erros de typecheck (`tsc -b`), lint sem emojis e build de produção Vite em 1.17s.

> **v2.26** registra a **Linha do Tempo de Aportes Históricos, Migrações 0043/0044 e Integridade Cirúrgica da TIR (XIRR)** (2026-09-04):
> - **(1) Diagnóstico e Saneamento de Duplicatas (Migration 0043)**: Identificada e corrigida a inconsistência que gerava TIR negativa (-33,9% a.a.) por acúmulo de marcos duplicados antigos na tabela `portfolio_contributions`. A migration 0043 executou limpeza por `PARTITION BY user_id` e instituiu o RPC de segurança;
> - **(2) Linha do Tempo de Aportes Históricos (Migration 0044 & RPC `create_historical_contribution`)**: Evolução do conceito de "Marco Zero Único" para uma linha do tempo completa. Permite registrar múltiplos marcos temporais anteriores ao app (início modesto, aportes em massa e consolidados) com validação de `auth.uid()` no servidor, calculando o tempo exato de maturação de cada bloco de capital e elevando a TIR anualizada da taxa diluída (~3,8% a.a.) para a faixa real ponderada de **~6,5% a 7,5% a.a.**;
> - **(3) Interface Reestruturada (`InitialPocketCostDialog`)**: Modal adaptativo `size="lg"` com card resumo do capital total do bolso em `MoneyText`, formulário de inclusão rápida em 3 campos (Data, Valor R$, Descrição), listagem interativa de marcos com exclusão com 1 clique e orientação didática sobre ponderação temporal;
> - **(4) Suíte 100% Verde e Build Limpo**: 280 arquivos de teste / 2.016 testes passando (100% verde), zero erros de typecheck (`tsc -b`), lint sem emojis e build de produção Vite em 1.30s.

> **v2.25** registra a **Migração 0042, Suporte a Datas Históricas de Investimentos e Recalibração Permanente do Marco Zero** (2026-09-04):
> - **(1) Migration SQL 0042 (`0042_portfolio_historical_dates.sql`)**: Flexibilização das constraints rígidas de data nas tabelas de investimentos (`portfolio_contributions`, `portfolio_dividends` e `portfolio_transactions`) para permitir datas históricas a partir de `2000-01-01` (eliminando a restrição de `>= 2026-01-01` que barrava o registro do início real da jornada de investimentos do usuário, como 2023, 2022);
> - **(2) Recalibração Contínua do Marco Zero do Bolso**: O banner e o acionador de Marco Zero agora permanecem sempre acessíveis no painel de aportes e no modal didático de rentabilidade (mesmo quando já configurados), permitindo ao investidor recalibrar custos iniciais e datas de corte a qualquer momento;
> - **(3) Blindagem de Inputs e Diálogos de Investimentos**: Resolução de interoperabilidade no `DatePicker` (suporte universal a `onValueChange` e `onChange`), eliminação de erros de cascading render / lifecycle no `InitialPocketCostDialog` e copy explicativo ressaltando a influência da data base no cálculo anualizado da TIR;
> - **(4) Suíte 100% Verde**: 276 arquivos de teste / 1.972 testes passando (100% verde), zero erros de typecheck (`tsc --noEmit`), lint estrito sem emojis e conformidade com design system.

> **v2.24** registra o **Motor Puro de TIR / Fluxo do Bolso (XIRR), Desacoplamento da Custódia Viva e Trio Estratégico de Rentabilidade** (2026-09-04):
> - **(1) Motor Puro de TIR (XIRR / Money-Weighted Return)**: Implementado solver numérico híbrido (Newton-Raphson com fallback para Bisseção Intervalar no range `[-0.999, 10.0]`) em `src/domain/portfolio/irr.ts` e 12 testes exaustivos em `irr.test.ts`. Anualização ponderada no tempo com salvaguarda para períodos $< 30$ dias;
> - **(2) Desacoplamento Matemático da Custódia Viva & P&L Histórico**: Eliminação da armadilha do turnover em `src/domain/portfolio/summary.ts`. O percentual da custódia viva reflete estritamente ativos em posse hoje vs seus custos médios, enquanto o passado completo vive no Resultado Econômico Histórico (P&L Total em R$);
> - **(3) Suporte ao Marco Zero do Bolso**: Compatibilidade com registro de aportes históricos anteriores ao início do app via `portfolio_contributions` sem inflar custos médios de ativos cotizados;
> - **(4) UI & UX Explicativa e Didática**: Dashboard de Investimentos (`ResumoTab`) atualizado com 4 KPIs balanceados (Patrimônio Total com retorno vivo, Saldo em Caixa, TIR do Bolso e Resultado Histórico P&L), Modal Didático "Modelos de Rentabilidade da Carteira" e TIR do Ativo no Sheet de Detalhes (`AssetDetailSheet`);
> - **(5) Suíte 100% Verde**: 276 arquivos de teste / 1.972 testes passando (100% verde), zero erros de typecheck (`tsc --noEmit`), lint estrito sem emojis e conformidade com design system.
...
> - **(3) Diagnóstico de Risco e Concentração Setorial (`concentration-risk.ts`)**: cálculo de dominância setorial (`topSectorDominance`), exposição agregada (`sectorExposure`) e disparo de alertas de risco de concentração quando um único setor excede os limites de segurança da carteira;
> - **(4) Dossiês e Relatórios Executivos Integrados**: atualização de `WealthTearSheetModal`, `PortfolioExecutiveReport` e do exportador multilivro `excel-export.ts` com detalhamento setorial, coluna de setor na tabela de custódia consolidada e parecer técnico automatizado destacando a prioridade setorial de aporte;
> - **(5) Suíte 100% Verde**: 218 arquivos / 1.651 testes passando (100% verde), typecheck estrito e ESLint com zero erros.

> **v2.00** registra as **Correções Críticas de Ergonomia no Wizard de Investimentos: Condicionamento de Sugestões a Metas, Feedback Visual Imediato na Seleção de Ativos e Suporte Nativo a Tickers de 1 Letra (ex: "O" - Realty Income)** (2026-08-24):
> - **(1) Recomendações Estritamente Condicionadas a Metas**: as sugestões de aporte no Wizard (`StepSelect` / `buildAporteSuggestions`) agora só são calculadas e rotuladas como "Recomendados para aporte" quando houver metas cadastradas (por ativo ou classe). Na ausência de metas, exibe-se um banner informativo orientando a configuração na aba Metas e listando os ativos em carteira para seleção manual sem rotulagem algorítmica indevida;
> - **(2) Feedback Visual de Seleção e Transição Fluida no Wizard**: implementação de destaque visual nítido no item ativo (`isSelected` com `ring-2 ring-primary bg-primary/10`, badge "Selecionado" e ícone de check), sincronização automática do termo de busca com o ticker selecionado e avanço direto para o **Passo 2: Posição Inicial** (`mode: "new_asset", step: 2`) ao escolher um novo ativo da busca/catálogo, eliminando a permanência redundante no Step 1;
> - **(3) Suporte Global a Tickers de 1 Caractere**: remoção da restrição de tamanho mínimo (`q.length >= 1`) em `tickers-catalog.ts` e atualização das regexes de classe internacional (`import-parser.ts`), moeda USD (`valuation.ts`) e cotações Yahoo (`quotes.ts`) para `/^[A-Za-z]{1,5}$/`, permitindo buscar, cadastrar e cotar ativos como `"O"` (Realty Income), `"T"` (AT&T), `"V"` (Visa), `"C"` (Citigroup) e `"F"` (Ford);
> - **(4) Motor de Ranking de Relevância na Busca de Ativos**: ordenação inteligente em `searchTickers` priorizando match exato de ticker em 1º lugar, seguido por prefixos e substrings;
> - **(5) Suíte 100% Verde**: 218 arquivos / 1.639 testes passando, zero erros de typecheck e zero violações de linter.

> **v1.99** registra as **Metas Hierárquicas de Investimentos (Classe $\rightarrow$ Setor $\rightarrow$ Ativo), Catálogo Curado com Inferência Inteligente e Aportes Fracionários em Moeda Estrangeira (USD) & Cripto** (2026-08-24):
> - **(1) Hierarquia de Alocação em 3 Níveis (Macro $\rightarrow$ Meso $\rightarrow$ Micro)**: transição do modelo antigo para o modelo Classe $\rightarrow$ Setor $\rightarrow$ Ativo. Metas setoriais são definidas relativamente à classe (somando até 100% da classe), preservando a integridade das metas de classe sem recálculos manuais;
> - **(2) Catálogo Curado & Inferência Setorial Automática (`inferSectorFromTicker`)**: inferência precisa por ticker e classe cobrindo Ações B3 (Financeiro/Bancos, Commodities/Petróleo, Elétricas/Saneamento, etc.), FIIs (Tijolo/Logística, Papel/CRI, etc.), Tesouro Direto por indexador econômico (*Pós-fixado (Selic / CDI)*, *Inflação (IPCA+)*, *Prefixado*), Criptoativos e Ativos Internacionais em Dólar (*Tech, Healthcare, REITs, ETFs Neutros All-World / S&P 500*);
> - **(3) Fracionamento e Precisão Decimal Adaptativa no Aporte**: cálculo de compras fracionárias para ativos em Dólar/USD (até 4 casas decimais) e Cripto (até 8 casas decimais), mantendo o mercado nacional (B3) em cotas inteiras ($\ge 1$ cota);
> - **(4) Sub-aba "Setores" nas Metas de Alocação (`TargetsTab`)**: seletor de classe ativa, barra de progresso e validação de soma de setores ($\le 100\%$), ações rápidas de normalização, 1/N e zeramento, e persistência em `group_targets` (`group_type: "sector"`);
> - **(5) Visualização Patrimonial por Setor (`ResumoTab`)**: novo modo "Por Setor" no gráfico de distribuição patrimonial Donut da carteira;
> - **(6) Suíte 100% Verde**: 218 arquivos / 1.636 testes passando (35 arquivos / 312 testes específicos de investimentos), typecheck e ESLint com zero erros.

> **v1.98** registra a **Evolução do Motor de PDF & Dossiês: Nomenclatura Automática Inteligente (`document.title`), Unificação Canônica da Fatura de Cartões e Exportação Executiva de Extratos e IRPF** (2026-08-24): **(1) Nomenclatura Contextual e Automática ao Salvar PDF**: atualização do hook `usePrint` e do contêiner `ReportDocumentLayout` para injetar temporariamente o nome sugerido para o arquivo PDF no `document.title` do navegador durante o ciclo de impressão (ex.: `Fatura_Nubank_2026-08.pdf`, `Informe_Rendimentos_IRPF_2026.pdf`, `Extrato_Consolidado_Investimentos.pdf`) e restaurar o título original do aplicativo automaticamente após a captura; **(2) Unificação Canônica da Fatura de Cartão (`cards-page.tsx`)**: migração da fatura de cartão para o contêiner editorial padrão `<ReportDocumentLayout size="2xl">` com acionamento unificado pelo cabeçalho nativo via `usePrint`, eliminando chamadas cruas a `window.print()` e duplicações manuais de marcação; **(3) Dossiê Fiscal & IRPF com Exportação para PDF (`PortfolioTaxReport`)**: integração de `ReportDocumentLayout`, `ReportHeader` e `ReportFooter` na visualização anual de IRPF, permitindo gerar a folha de informe completa com Bens & Direitos, Rendimentos Isentos e Tributação Exclusiva pronta para arquivamento ou envio ao contador; **(4) Extrato da Carteira com Impressão Executiva (`PortfolioStatementDialog`)**: adição de ação no cabeçalho nativo e portal de impressão `<PrintSheet>` formatando os lançamentos de compras, vendas, proventos e splits em tabela limpa e paginada; **(5) Autenticação Automática de Integridade no `ReportFooter`**: geração de código de autenticidade contábil (`ID: BR-YYYYMMDD-XXXX`) em todos os rodapés institucionais; **(6) Numeração Nativa de Páginas no Paged Media**: adição de `@bottom-right` com contadores `counter(page)` e `counter(pages)` nas regras de `@page` em `globals.css`; **(7) Suíte 100% Verde**: 215 arquivos / 1.602 testes passando, build de produção e lint limpos.

> **v1.97** registra a **Refatoração do Motor de PDF & Dossiês: Margens Físicas A4 Seguras, Paginação Inteligente Anti-Órfãos e Ritmo Vertical Contínuo** (2026-08-24): **(1) Margens Físicas Canônicas (`@page`)**: substituição do antigo `margin: 0` por `@page { size: A4 portrait; margin: 15mm 12mm 15mm 12mm; }` em `src/styles/globals.css`, eliminando o colapso de margem a partir da página 2 (onde elementos ficavam colados no topo e base física da folha) e padronizando 15mm de respiro superior/inferior e 12mm lateral em 100% das páginas geradas sem reintroduzir cabeçalhos/rodapés do navegador; **(2) Paginação Inteligente e Prevenção de Fragmentação**: aplicação de regras rigorosas `break-inside: avoid !important` em `tr`, `td`, `th`, `section`, `article`, `figure`, cards arredondados e regiões analíticas, impedindo que cards, gráficos ou linhas de tabela sejam cortados horizontalmente ao meio; **(3) Prevenção de Títulos Órfãos**: regras `break-after: avoid !important` em todos os níveis de título (`h1`–`h6`, `header`, `.report-section-header`) garantindo que nenhum título de seção fique isolado no rodapé da folha; **(4) Repetição Automática de Cabeçalhos de Tabela**: `thead { display: table-header-group !important; break-after: avoid !important; }` garantindo repetição limpa do cabeçalho de colunas caso uma tabela se estenda por múltiplas páginas; **(5) Ritmo Vertical e Aproveitamento Harmônico**: remoção de vácuos excessivos com espaçamento vertical padronizado (`gap: 16px` no documento, `gap: 8px` em seções); **(6) Suíte 100% Verde**: 215 arquivos / 1.602 testes passando, build de produção e lint limpos.

> **v1.96** registra a **Auditoria, Refatoração Estrutural, Responsividade Mobile/Desktop e Padronização de PDF do Módulo de Relatórios e Investimentos** (2026-08-24): **(1) Resolução da Quebra de Desktop em Modais (`ModalSize` Expandido)**: expansão do catálogo de tamanhos no primitivo `Modal` (`src/components/ui/modal.tsx`) com suporte nativo a `"2xl"` (`lg:max-w-5xl` = 1024px), `"3xl"` (`lg:max-w-6xl` = 1152px) e `"full"` (`lg:max-w-7xl` = 1280px), eliminando a trava de cascata CSS que comprimia todos os relatórios e dossiês patrimoniais a 448px (`lg:max-w-md`) no desktop; **(2) Unificação Canônica de `ReportDocumentLayout`**: configuração padrão com `size="2xl"`, padding editorial balanceado (`p-4 sm:p-6 lg:p-7`) e migração do `PortfolioExecutiveReport` para o layout unificado com `usePrint()`, eliminando duplicações de cabeçalho e botões soltos no corpo; **(3) Otimização dos Diálogos de Investimentos**: `PortfolioTaxReport` (`size="2xl"` com botões de ano-base padronizados e eliminação de rolagens restritivas aninhadas), `PortfolioStatementDialog` (`size="3xl"` para 7 colunas de extrato com respiro pleno no desktop), `PortfolioImportDialog` (`size="2xl"`) e `TransactionListDialog` (`size="lg"`); **(4) Alinhamento de Contêiner com `PageShell`**: remoção de duplo padding na `ReportsPage` e nos estados de Skeleton e ErrorState (`<div className="flex flex-col gap-6 w-full min-w-0">`) com `refetch()` assíncrono TanStack; **(5) Responsividade de Tabelas e Motor de PDF A4**: larguras protegidas em `ReportTable` (`DataList`), regras `@media print` no `globals.css` com `overflow: visible`, `break-inside-avoid` e distribuição percentual precisa nas 8 colunas de custódia evitando cortes na margem direita em A4 Portrait; **(6) Suíte 100% Verde**: 215 arquivos / 1.602 testes passando, build de produção e lint limpos.
...
> **v1.80** registra a **Central Unificada de Relatórios de Consultoria Patrimonial & Exportação Multi-Abas (.xlsx / A4)** (2026-08-23): **(1) Centralização Total no Hub `/relatorios`**: unificação de todos os relatórios financeiros e patrimoniais sob a rota canônica `/relatorios` com 4 abas estruturadas (*Finanças & DRE*, *Investimentos & Alocação*, *Balanço 360°*, *Fiscal & IRPF*), descontinuando a redundância na área de investimentos e preservando navegação por deep linking (`?aba=investimentos`, `?aba=fiscal`); **(2) Dossiês Executivos A4 de Consultoria**: 4 visões de impressão editoriais integradas (`WealthTearSheetModal`, `DividendFreedomModal`, `ConsolidatedWealthModal`, `TaxFacilitatorModal`) com pareceres automatizados, risco de concentração, índice de liberdade financeira, efeito bola de neve e discriminação de Bens & Direitos para IRPF com cópia em 1-clique; **(3) Caderno Excel Multi-Abas com Fórmulas**: gerador `src/services/excel-export.ts` produzindo arquivo XML Spreadsheet 2003 (.xlsx) com 5 abas formatadas (*Resumo Executivo, Custódia e Ativos, Extrato de Proventos, DRE Financeiro, Dívidas e Passivos*); **(4) Motores Puros de Domínio**: `allocation-gaps.ts`, `concentration-risk.ts`, `freedom-index.ts` e `consolidated-balance.ts` 100% testados com dados vazios e parciais; **(5) Suíte 100% Verde**: 198 arquivos / 1.555 testes passando, zero erros de lint e typecheck.
...
> **v1.75** registra a **Calculadora de Aporte Inteligente com Distribuição Hierárquica Classe $\rightarrow$ Ativo, RPC Transacional Atômico e Suporte Fracionário a Cripto** (2026-08-23): **(1) Estabilização Macro por Classe**: alocação prioritária do aporte entre classes deficitárias com base no déficit relativo das metas de classe, garantindo a preservação dos objetivos patrimoniais sem canibalização de verbas entre classes distintas; **(2) Alocação Micro Equilibrada por Ativo**: verba destinada à classe distribuída estritamente entre seus membros com modo de equiponderação $1/N$, eliminando distorções onde ativos já sobrealocados atraíam mais aporte; **(3) Transbordamento & Precisão Decimal em Criptoativos**: resíduos internos de classe retornam ao pool para atender a próxima classe defasada, e suporte nativo a cotas decimais em Cripto (até 8 casas); **(4) Diagnóstico de Ativos Ignorados**: painel retrátil no resultado (`AporteResult`) detalhando motivos de ativos não contemplados (*Sem cotação, Na meta, Sem meta*); **(5) RPC Transacional no PostgreSQL (`execute_portfolio_batch_aporte`)**: gravação atômica em 1-clique via migração `0025`, atualizando simultaneamente posições em `portfolio_assets` (PM ponderado e cotas), lançando compras no extrato (`portfolio_transactions`) e registrando o aporte financeiro em `portfolio_contributions`. Suíte 100% verde.
> **v1.74** registra a **Taxa de Poupança com Aportes, Precificação Direta de Renda Fixa/Tesouro Direto, Calculadora Flutuante nos Modais de Investimentos e Desbloqueio do Assistente de Aportes** (2026-08-23): **(1) Taxa de Poupança & Visão Geral**: fórmula de poupança em `computeOverview` corrigida para $\frac{\text{income} - \text{expense}}{\text{income}} \times 100$ para refletir o esforço total de poupança/investimento sem zerar após aportes, e inclusão de aportes no gráfico de fluxo diário (`DailyFlowChart`) e no fechamento do período em `ReportsPage`; **(2) Renda Fixa em Valor Completo (Preço Inicial e Saldo Atual)**: ativos de Renda Fixa (CDB, LCI, LCA, CRI, CRA, Debêntures) e Tesouro Direto sem cotas fracionárias nem PM unitário artificial no cadastro (`AssetFormDialog`, `StepNewPosition`) e nos detalhes da carteira (`PositionTable`, `AssetDetailSheet`); **(3) Tesouro Direto com Modo de Precificação Configurável**: padrão em Valor Completo (Renda Fixa) com opção de alternância para Preço Médio/Cotas via tag `[PRICING:UNIT]`; **(4) Aporte e Resgate sem Cotas no Wizard & Quick Transaction**: campos de valor monetário direto com atalhos rápidos de percentual (25%, 50%, 75%, 100%), remoção da operação de Split para Renda Fixa e validação reativa no `canProceed` desbloqueando o avanço de passos; **(5) Calculadora Flutuante (`FloatingCalculator`)**: prop `showCalculator` habilitada nos modais `AssetFormDialog`, `InvestmentWizard`, `CashFormDialog`, `DividendFormDialog`, `QuickTransactionSheet` e `AssetSplitDialog` com injeção direta de valores calculados em `MoneyInput`. Suíte **1510 testes / 193 arquivos** (100% verde).
> **v1.73** registra a **Ergonomia da Tabela de Posições, Interatividade de Linha Inteira, Hover Padronizado no Ativo e Eliminação de 3 Pontinhos** (2026-08-23): **(1) Clique na Linha Inteira da Tabela**: suporte a `onRowClick` no `DataList` integrado à `PositionTable`, permitindo abrir o modal de detalhes do ativo (`AssetDetailSheet`) com clique ou teclado em qualquer área da linha no desktop; **(2) Hover Padronizado e Acessível no Ativo**: botão do ativo (ticker/classe) com affordance visual idêntica ao botão de preço (`hover:bg-surface-hover/80`, `rounded-md`, transição suave e `group-hover:text-primary`); **(3) Isolamento de Eventos no Preço**: `e.stopPropagation()` garantindo que o clique na cotação acione exclusivamente o diálogo de ajuste manual de preço (`ManualPriceDialog`); **(4) Despoluição Visual com Eliminação dos 3 Pontinhos**: remoção da coluna de 3 pontinhos (`PositionRowActions`) na tabela desktop e nos cards mobile, visto que as ações de edição, exclusão, aportes e proventos já estão centralizadas no modal de detalhes; **(5) Testes Automatizados**: testes unitários de clique na linha, clique no ativo e isolamento do clique no preço em `position-table.test.tsx` (100% verde).
> **v1.72** registra o **Ícone Dinâmico, Destaque Ativo Padronizado e Transição Spring Pop no Slot "Mais" da BottomNav Móvel** (2026-08-22): **(1) Ícone Dinâmico por Subpágina**: detecção contextual da rota ativa para subpáginas do menu Mais (`/dividas`, `/investments`, `/relatorios`, `/insights`, `/orcamentos`, `/lembretes`, `/configuracoes`), substituindo o ícone dos 3 pontinhos (`Ellipsis`) pelo ícone específico da página ativa (`HandCoins`, `ChartLine`, `ChartPie`, `Lightbulb`, `PiggyBank`, `Bell`, `Settings`); **(2) Destaque Ativo Padronizado**: slot 5 recebe o estilo ativo `text-primary-strong` quando em subpáginas do Mais ou na raiz `/mais`, igualando-se ao comportamento visual das abas principais (`/`, `/transacoes`, `/cartoes`); **(3) Transição Spring Pop**: animação fluida `animate-spring-pop transform-gpu` disparada via `key={moreSubItem?.path ?? "mais"}` com supressão graciosa em acessibilidade (`data-motion="reduced"` e `prefers-reduced-motion`); **(4) Acessibilidade e Testes**: `aria-label` contextualizado (`"Mais (Nome da Página)"`) e 11 testes unitários dedicados em `bottom-nav.test.tsx` (100% verde).
...
> **v1.64** registra o **Motor Especializado de Extração em Linguagem Natural para o Quick-Paste** (2026-08-18): **(1) Extrator de Entidades Financeiras em Texto Livre**: módulo `src/domain/reconciliation/parsers/text-parser.ts` com `parseNaturalLanguageLine` capaz de isolar datas (ISO, `DD/MM/AAAA`, `DD/MM`, meses por extenso como `15 de agosto` e termos relativos como `hoje`, `ontem`), valores monetários (`R$ 45,90`, `50 reais`, `por 250,00`) e parcelas (`3x`, `(02/05)`), gerando matrizes normalizadas de 3 colunas `[Data, Descrição, Valor]` para frases e linhas sem delimitadores tabulares tradicionais; **(2) Resiliência Monetária sem Split Ingênuo**: eliminação da divisão por vírgula que fatiava centavos (`45,90` $\rightarrow$ `["45", "90"]`), adotando regexes estritas para padrões numéricos brasileiros e internacionais; **(3) Higienização de Conectivos Conversacionais**: remoção inteligente de verbos introdutórios (`gastei`, `comprei`, `paguei`) e preposições residuais (`no`, `na`, `por`, `reais`), preservando o nome real do estabelecimento e as parcelas; **(4) Sniffer e Extensão de Tipos**: `normalizeDateToISO` e `parseAmountToCents` em `type-sniffer.ts` expandidos com suporte a meses nominais e datas relativas; **(5) Testes Unitários de Domínio**: expansão de `reconciliation.test.ts` cobrindo linguagem natural, separadores por traço/pipe e integração de ponta a ponta via `parseStatementInput`. Suíte **1305 testes / 162 arquivos** (100% verde).
> **v1.63** registra o **Motor Client-Side de Cotações Automáticas, Fallback em Cascata (Brapi + AwesomeAPI + Yahoo), Auto-Sync ao Cadastrar Ativo e Prevalência Estrita de Preço Manual** (2026-08-18): **(1) Serviço de Cotações em Tempo Real no Cliente**: módulo `src/services/quotes.ts` com busca assíncrona tolerante a falhas e suporte nativo a CORS no navegador via **Brapi** (`brapi.dev`) para B3 (ações/FIIs/BDRs), **AwesomeAPI** (`economia.awesomeapi.com.br`) para câmbio (`USD-BRL`/`USDBRL=X`) e **Yahoo Finance Chart API** em cascata; **(2) Auto-Sync Imediato ao Cadastrar Ativo**: `useCreatePortfolioAsset` (`src/state/queries/use-portfolio.ts`) dispara a busca de cotação online imediatamente em segundo plano para o novo ticker criado, persistindo a cotação em `asset_prices` e atualizando o badge para *"cotação"* na mesma hora; **(3) Auto-Sync e Botão de Atualização em ResumoTab**: hook `useSyncQuotes` integrado à `ResumoTab` com sincronização automática na montagem para ativos sem cotação carregada e botão explícito *"Atualizar cotações"* com estado de loading animado (`RefreshCw`) e feedback em toast; **(4) Prevalência e Não-Zeramento de Preço Manual**: `listAssetPrices` e `setAssetPriceFromApi` em `src/data/repositories/asset-prices.ts` com `consolidateAssetPrices` garantindo que overrides manuais nunca sejam sobrescritos ou zerados por falhas de API; **(5) Parsers Puros e Isolamento**: motor puro `src/domain/portfolio/quotes.ts` com `parseBrapiResponse`, `parseAwesomeApiResponse`, `parseYahooChartResponse` e `normalizeTickerForBrapi` isolados e testados. Suíte **1304 testes / 162 arquivos** (100% verde).
> **v1.62** registra o **Sistema Completo de Notificações, Sininho no Header com Popover Rápido, Gestão de Preferências e Ações em Lote** (2026-08-18): **(1) Sininho no Header com Indicador Numérico**: botão `NotificationsButton` com badge dinâmico (com distinção de tom para contas atrasadas `critical` ou vencendo hoje `warning`); **(2) Popover de Notificações Rápidas**: `NotificationsPopover` integrado ao `Popover` Radix (`src/components/ui/popover.tsx`) com lista compacta das maiores urgências, ação de marcar como lido, deep-links contextuais e atalho para a central completa; **(3) Conexão Real com Preferências no Banco**: persistência de `reminders_enabled`, `reminder_days_before_debt` e `reminder_days_before_bill` via `user-preferences.ts`, `useUserPreferences` e `useUpdateReminderPreferences`; **(4) Configurações de Lembretes**: aba dedicada *Lembretes* na página `/configuracoes` com toggles e `NumberStepperInput`; **(5) Central de Lembretes Aprimorada**: hook unificado `useReminders`, botão *Marcar todas como lidas* (`markAllRemindersAsRead`) e abas de filtros por categoria. Suíte **1273 testes / 160 arquivos** (100% verde).
> **v1.61** registra a **Reconciliação Bidirecional de Faturas de Cartão, Prevenção de Duplicatas e Sniffer Inteligente** (2026-08-18): **(1) Prevenção de Duplicatas na Importação**: transações correspondidas com despesas existentes (`exact_match` ou `probable_match`) vêm desmarcadas por padrão (`selected: false`), garantindo que apenas lançamentos faltantes de fato (`unmatched_new`) sejam importados; **(2) Reconciliação Bidirecional (Sobras no App)**: motor `reconcileStatementTransactions` identifica despesas cadastradas no app na competência que não constam no extrato bancário (`unmatchedExistingExpenses`), com alerta contextual no diálogo e aba dedicada `No App apenas (X)` para auditoria; **(3) Calibração de Scoring**: mesmo valor + mesma data pontua 85% $\rightarrow$ classificado como *Conciliado* (`exact_match`), mesmo com divergências leves no nome do estabelecimento; **(4) Sniffer de Colunas com Cardinalidade e Cabeçalhos**: colunas fixas de titular/portador (`GABRIEL I S SALES`) penalizadas e estabelecimentos priorizados por alta cardinalidade; **(5) Mapeamento Manual**: Passo 2 aplica diretamente a escolha do usuário via `buildTransactionsFromRows`. Suíte **1254 testes / 154 arquivos** (100% verde).
> **v1.60** registra o **Pleno Funcionamento, Preço Manual, Execução de Aporte, Busca Local e Yield on Cost (YoC) na Carteira de Investimentos** (2026-08-18): **(1) Interface de Preço Manual / Override de Cotação**: diálogo `ManualPriceDialog` permitindo fixar preço manual (BRL/USD) ou reverter para a cotação automática da API de mercado (`useSetManualPrice` / `useRemoveManualPrice`), acessível via clique no badge de preço e no menu de ações da `PositionTable`; **(2) Execução Direta do Aporte Sugerido**: botão *"Lançar compras no extrato"* na aba Aporte (`AporteTab` / `AporteResult`) com modal de confirmação e inserção atômica em lote no ledger via `createPortfolioTransactionsBatch` (`useCreatePortfolioTransactionsBatch`); **(3) Busca Rápida e Filtros por Classe**: barra de pesquisa textual instantânea por ticker/classe e chips de filtro por classe integrados à ordenação e aos cards mobile da `PositionTable`; **(4) Yield on Cost (YoC)**: motor puro `assetYieldOnCostPct` (`domain/portfolio/summary.ts`) calculando o retorno acumulado de proventos sobre o custo de aquisição, exibido com destaque nos cards mobile e na tabela; **(5) Barrels & Governança**: barrels `index.ts` em `src/features/investments` e `src/features/portfolio/components`. Suíte **1249 testes / 154 arquivos** (100% verde).
> **v1.59** registra a **Reorganização, Limpeza Visual e Harmonização da Página de Investimentos** (2026-08-18): **(1) Hierarquia de Informação na Aba Resumo**: elevação das **Posições da Carteira** para logo após os KPIs executivos, integrando o botão `Adicionar ativo` ao cabeçalho da seção e movendo os gráficos de Donut (*Alocação por Classe* e *Alocação por Ativo*) para a seção subsequente de análise; **(2) Eliminação de Ruído Cognitivo**: remoção de textos técnicos redundantes nos cabeçalhos; **(3) Organização da Aba Metas**: estruturação em sub-abas dedicadas (`Metas por Ativo` e `Classes & Travas de Risco`), eliminando o scroll vertical excessivo; **(4) Harmonização de Superfícies em Aporte e Proventos**: padronização de contêineres e seletores de mês com os tokens de superfície `rounded-2xl border-border/80 bg-surface/90 shadow-xs`. Suíte 100% verde.
> **v1.58** registra a **Auditoria Completa de Consistência Visual, Design System e Paridade Mobile-First** (2026-08-18): **(1) Paridade Total em Dívidas & Financiamentos**: restabelecimento das ações de cabeçalho no mobile (botões adaptativos `Nova dívida` e `Financiamento`), inclusão de aba dedicada `Financiamentos` com `EmptyState` temático e CTA de criação mesmo sem contratos prévios, eliminando a inacessibilidade em smartphones; **(2) Zero Diálogos Nativos em Contratos**: substituição de `window.confirm` por `ConfirmDialog` do Design System no `LoanCard`; **(3) Padronização de Headers e Superfícies**: botões de cabeçalho responsivos e fluidos em `CategoriesPage` (`Nova categoria`), elevação do resumo de orçamentos em `BudgetsPage` para o padrão de superfície `rounded-2xl border-border/80 bg-surface/90 shadow-xs` da Home. Suíte 100% verde.
> **v1.57** registra a **preservação de rota ativa e contexto em gatilhos globais de criação/ação rápida e wizards** (2026-08-17): **(1) LaunchWizard em Overlay Contextual**: refatoração do wizard de lançamento de transações para operar como modal sobreposto (`Modal` Radix com bottom-sheet mobile e diálogo centralizado desktop) montado no `PageShell` e disparado via deep link declarativo `?novo=transacao` ou controle de estado sem alterar ou forçar desvio da rota ativa; **(2) Conclusão e Descarte sem Desvios de Rota**: encerramento do wizard (sucesso ou cancelamento/descarte) preserva o usuário exatamente na página de origem (Início, Transações, Cartões, Relatórios, etc.) com seus parâmetros e scroll intactos; **(3) FAB Contextual Dinâmico**: FAB da `BottomNav` preserva o `pathname` e parâmetros de busca da tela atual para todas as ações contextuais (`?novo=transacao`, `?novo=cartao`, `?novo=divida`, `?novo=categoria`); **(4) Compatibilidade e Sincronização em Segundo Plano**: redirecionamento transparente de `/transacoes/novo` para `/transacoes?novo=transacao` preservando atalhos PWA e bookmarks, com invalidação automática de cache do TanStack Query em segundo plano. Suíte **1219 testes / 149 arquivos** (100% verde).
> **v1.56** registra a **implementação da Fase 32 — Recorrências, Rendas Parceladas & Operações em Grupo** (2026-08-17): cadastro formal de recorrências (template `recurrences` + materialização sob demanda idempotente com `recurrence_skips`, frequências mensal/semanal/trimestral/anual com clamp de fim de mês, fim por data ou nº de ocorrências), parcelamento de rendas (1–60, invariantes D12), edição/exclusão em grupo (`single | all | subsequent`) para parcelas e recorrências de despesas e rendas (valor em grupo: parcelas mantêm valor individual consistente com `base_amount`; recorrências aceitam valor em grupo), wizard com bloco de recorrência + parcelas de renda, seção "Recorrentes" no extrato, insights sem duplicidade (linhas com `recurrence_id` fora da detecção histórica). Motor puro `domain/recurrences` (+ testes), migration `0013`, 8 RPCs transacionais (2 RPCs individuais do plano consolidados no `single` dos RPCs de grupo — DRY), estado otimista padrão F30, docs atualizadas (spec §3.1/§3.2.5/§3.2.7). Suíte **1202 testes / 145 arquivos** (100% verde). **v1.55** registra o **refinamento de UX de cartões, responsividade de visualização/impressão e ajustes de navegação** (2026-08-17): **(1) Responsividade de Faturas e Fechamento Mensal**: `CardInvoicePrintView` e `MonthlyClosePrintView` adaptados para dispositivos móveis com tabelas em contêineres com `overflow-x-auto` e largura mínima sem quebra de colunas, cabeçalhos flexíveis (`flex-col sm:flex-row`) e contenção nos modais, mantendo renderização contínua e sem quebras na impressão/PDF via `print:overflow-visible` e `print:flex-row`; **(2) Centralização da Exclusão de Cartões**: remoção do botão de exclusão rápida da `CreditCardWallet`, unificando e protegendo a exclusão exclusivamente dentro do diálogo de edição `CardFormDialog`; **(3) FAB da BottomNav em Cartões**: FAB contextual de cartões direcionado para o wizard de lançamento (`/transacoes/novo`). Suíte **1160 testes / 142 arquivos** (100% verde).
> **v1.54** registra a **proposta da Fase 32 — Recorrências, Rendas Parceladas & Operações em Grupo** (2026-08-17): cadastro formal de recorrências (template + materialização sob demanda, frequências mensal/semanal/trimestral/anual, fim por data ou nº de ocorrências), parcelamento de rendas (1–60, invariantes D12) e edição/exclusão em grupo (`single | all | subsequent`) para parcelas e recorrências de despesas e rendas (ver seção Fase 32 em §3). **v1.53** registra o **refinamento estético definitivo do PWA — Splash Screen limpo em fundo branco, ícone ampliado e estratégia híbrida de ícones PWA** (2026-08-17): **(1) Splash Screen puro em fundo branco com ícone centralizado**: `#app-splash` e `manifest.webmanifest` padronizados com fundo branco sólido (`#FFFFFF`), ícone de 144px perfeitamente centralizado com animação de pulso sutil, sem nenhum elemento textual ou barra de progresso; **(2) Estratégia híbrida de ícones PWA**: ícones `purpose: "any"` (desktop/iOS/dispositivos modernos) gerados com fundo 100% transparente (icon-only); ícones `purpose: "maskable"` (Android adaptive) gerados sobre caixa/fundo branco sólido (`#FFFFFF`, safe zone 80%) para recorte perfeito pelo sistema operacional; **(3) Sincronização contínua de tema**: bootstrap inline em `index.html` e reatividade no `ThemeProvider` (`theme-provider.tsx`) mantendo `<meta name="theme-color">` e a barra de título em sincronia com o tom do app; utilitários `.app-region-drag` e `.app-region-no-drag` no cabeçalho do `PageShell`. Suíte **1160 testes / 142 arquivos** (100% verde).
> **v1.52** registra a **simplificação ergonômica de transações e descontinuação do Swipe-to-Delete** (2026-08-17): **(1) Whole-Element Interaction em Transações**: remoção completa de gestos de arrasto lateral (`useSwipeAction`, handlers de ponteiro e contêineres ocultos de overflow) em `TransactionRow`; clique direto em qualquer ponto da linha aciona a abertura do diálogo de detalhes com feedback tátil (`triggerHaptic("light")`); **(2) Diálogos de Detalhes Simplificados**: `ExpenseDetailDialog` e `IncomeDetailDialog` refatorados com a remoção da prop `openDeleteConfirm` e do estado `deleteIntent`, centralizando a exclusão e confirmação modal diretamente nos botões internos de ação dos diálogos; **(3) Limpeza e Descontinuação**: remoção do hook `useSwipeAction` e testes associados, eliminando código morto e dependências não utilizadas. Suíte **1159 testes / 142 arquivos** (100% verde).
...
- [x] **FAB da BottomNav contextual por página:** o `+` agora abre a criação do contexto atual — Início/Transações/Cartões → wizard de lançamento (`/transacoes/novo`); Dívidas → formulário de dívida (`?novo=divida`); Categorias → formulário de categoria (`?novo=categoria`); demais páginas caem no wizard de lançamento (fallback). Implementado com o hook reutilizável `useCreateDeepLink` (`?novo=<key>` — estado derivado da URL, sem setState em effect; fechar limpa o parâmetro com replace) + aria-label dinâmico do FAB. + 5 testes do hook + 3 de alvo por rota.
...
**Objetivo:** formalizar lançamentos recorrentes (despesas e rendas), habilitar o parcelamento de rendas e estender a edição/exclusão em grupo (`single | all | subsequent`) a parcelas e recorrências de despesas e rendas.

**Decisões de produto (2026-08-17):**
1. **Modelo:** template (`recurrences`) + **materialização sob demanda** — o cliente calcula as ocorrências do mês (D12) e o RPC `materialize_recurrences` insere as linhas faltantes de forma idempotente; `recurrence_skips` impede a regeneração de ocorrências excluídas individualmente.
2. **Frequências:** `monthly | weekly | quarterly | yearly` (enum extensível), com clamp de dia de fim de mês.
3. **Fim sempre definido:** `end_date` XOR `occurrences_total` (CHECK) — recorrência sempre finita.
4. **Operações em grupo:** estendem o padrão de exclusão atual (single/all/subsequent) para **edição** e para **rendas**; valor editável em grupo **apenas em recorrências** (sem invariante de soma); em parcelas o valor permanece individual via RPC que mantém `value` + `base_amount` consistentes (corrige o bug latente do `updateExpense` direto, que quebra o invariante sem tocar `base_amount`).

**Entregas (etapas):**
1. **Domínio** — `src/domain/recurrences/` (motor de ocorrências por frequência + `resolveOccurrenceDeleteIds`) com testes (frequências, clamp de fim de mês, fim por data/contagem, modos de exclusão).
2. **Banco** — migration `0013`: tabelas `recurrences` + `recurrence_skips`; colunas `recurrence_id`/`occurrence_number` em `expenses`/`incomes`; parcelamento em `incomes` (`installments_total/number/group_id` + CHECKs espelhando `expenses`); **8 RPCs** — `create_recurrence`, `materialize_recurrences`, `delete_recurrence_occurrences`, `update_recurrence_occurrences`, `create_income_installments`, `delete_income_installments`, `update_income_installments_group`, `update_expense_installments_group` (os 2 RPCs individuais do plano foram consolidados no modo `single` dos RPCs de grupo — a edição individual de parcela passa pelo mesmo caminho e mantém `value`+`base_amount` consistentes); wrappers em `src/data/rpc.ts`; tipos em `schema.ts`/`database.ts`.
3. **Estado** — query `useRecurrences`; materialização lazy idempotente nos hooks de listagem; mutations otimistas (padrão F30) com rollback/toast.
4. **Wizard** — parcelas para renda; bloco "Recorrência" (frequência + fim por data ou nº de ocorrências) com prévia das ocorrências na revisão.
5. **Diálogos** — `ExpenseDetailDialog`/`IncomeDetailDialog` com seletor de escopo de edição e exclusão em grupo (valor desabilitado no modo grupo para parcelas, com hint "editar o total = excluir e refazer").
6. **Lista & Insights** — seção "Recorrentes" no extrato (ícone `Repeat`, subtítulo `x/N`); detecção histórica de insights ignora linhas com `recurrence_id` (recorrência formal não é "detecção").
7. **Docs & QA** — atualizar `ESPECIFICACAO_TECNICA.md` (§3.1, §3.2.5, novo §3.2.7), `PROJECT_STRUCTURE.md`, `ARCHITECTURE.md`; typecheck/lint/suíte; revisão desktop/mobile nos temas.

**Arquivos:** `src/domain/recurrences/*` (+ testes) · `supabase/migrations/20260101000013_recurrences.sql` · `src/data/rpc.ts` · `src/types/schema.ts` · `src/types/database.ts` · `src/state/queries/use-recurrences.ts` · `src/state/mutations/*` · `src/features/transactions/wizard/*` · `src/features/transactions/components/expense-detail-dialog.tsx` · `src/features/transactions/components/income-detail-dialog.tsx` · `src/features/transactions/pages/transaction-list-page.tsx` · `src/domain/insights/recurrences.ts`.

**✅ DoD (critérios de aceite):**
- Despesa recorrente: cadastro formal (4 frequências, fim por data ou nº de ocorrências), materialização idempotente e lazy.
- Renda parcelada (1–60, invariantes D12: soma = valor, datas ≥ APP_START_DATE) e renda recorrente.
- Edição/exclusão em grupo nos 3 modos para parcelas e recorrências de despesas e rendas (rendas automáticas `source_ref` permanecem somente-leitura).
- Edição individual de parcela nunca quebra soma/`base_amount` (teste de invariante no RPC).
- Insights: recorrências formais não aparecem como "detecção" duplicada.
- Testes de domínio/RPC/UI; typecheck/lint/build limpos; docs atualizadas; revisão desktop + mobile nos temas.
...
### Fase 34 — Importação e Reconciliação Inteligente de Extratos Bancários & Transações Gerais (Conta Corrente / PIX / TED)

> **Status:** ✅ Concluída (v1.65 — 2026-08-18) — motor puro `reconcileBankTransactions`, blindagens contra 6 fragilidades, RPC transacional `import_bank_transactions` e diálogo completo em 3 passos no app.

**Objetivo (Trilha A / Transações & Inteligência):** permitir a importação de extratos de conta corrente de qualquer instituição financeira (OFX SGML/XML nativo bancário, CSV multi-encoding com auto-delimitador e suporte a formato `D/C`, ou Quick-Paste de texto corrido do Internet Banking), conciliando simultaneamente **Despesas à vista/PIX/Débito/Boleto** e **Receitas**, com predição automática de categorias, deduplicação ordinal SHA-256 e blindagens nativas contra as 6 fragilidades financeiras.

**Proteção e Mitigação das 6 Fragilidades de Negócio:**
1. **Anti Double-Counting de Faturas:** heurística pura `isCreditCardPayment` que identifica pagamentos de fatura bancária (`PAGTO CARTAO`, `PAGTO FATURA`, `DEB AUT CARTAO`, `FATURA NUBANK/ITAU/INTER`) e os marca como `ignoredByDefault: true` (desmarcados por padrão), impedindo que o pagamento da fatura seja computado como uma nova despesa se os gastos individuais do cartão já estiverem registrados.
2. **Anti Inflação de Transferências e Aportes:** heurística `isInternalTransferOrInvestment` que detecta palavras-chave de transferências entre contas e aportes em corretoras (`APLICACAO CDB`, `RESGATE`, `TRANSF MESMA TITULARIDADE`, `XP/BTG/CLEAR`), marcando-os como desmarcados por padrão para não distorcer o fluxo de caixa, a renda e a taxa de poupança.
3. **Resiliência a Formatos de CSV (Coluna D/C e Colunas Duplas):** extensão do sniffer estatístico `type-sniffer.ts` para detectar colunas indicadoras de Débito/Crédito (`D/C`, `Debito/Credito`) ou colunas duplas (`Entrada`/`Saida`), garantindo atribuição precisa de sinal e permitindo inversão rápida na UI.
4. **Resiliência a Descrições Genéricas de PIX:** matching calibrado com 85 pontos (`exact_match`) para combinação de **Valor exato em centavos + Data idêntica**, casando o PIX da conta com o lançamento manual mesmo com textos divergentes (ex.: nome da pessoa física no banco vs descrição no app).
5. **Tratamento de Estornos e Devoluções:** identificador `isRefundOrReturn` para reconhecer devoluções de PIX/débito e evitar que sejam computadas como novas receitas salariais.
6. **Isolamento de Métodos de Pagamento:** reconciliação de saídas da conta corrente restrita a `payment_method != 'credit_card'`, prevenindo colisões entre compras do débito e compras parceladas no crédito.

**Organização da Implementação em 4 Etapas:**
1. **Etapa 34.1 — Motores Puros de Domínio (`src/domain/reconciliation/`):**
   - Tipos e esquemas Zod (`types.ts`): `BankTransactionItem`, `BankReconciliationResult`.
   - Sanitização de extrato de conta corrente e heurísticas anti-fragilidade (`clean.ts`).
   - Extensão do `type-sniffer.ts` para suporte a colunas indicadoras de tipo (`D/C`).
   - Motor de reconciliação de conta corrente `reconcileBankTransactions`: separação de débitos (candidatos a despesa à vista) e créditos (candidatos a receita), scoring multidimensional contra `expenses` e `incomes` existentes no mês, e listagem bidirecional de sobras no app (`unmatchedExistingExpenses` e `unmatchedExistingIncomes`).
   - Testes unitários puros com Vitest (`bank-reconciliation.test.ts`).
2. **Etapa 34.2 — Infraestrutura de Banco & RPC Transacional:**
   - Migration `supabase/migrations/20260101000016_bank_statement_import.sql`: colunas `statement_hash` e `imported_from_statement` na tabela `incomes` + índice condicional único `idx_incomes_user_statement_hash`.
   - RPC PostgreSQL `import_bank_transactions`: validação de `APP_START_DATE` (`>= 2026-01-01`), inserção atômica em lote de despesas (`expenses`) e receitas (`incomes`) com idempotência `ON CONFLICT DO NOTHING` e registro em `audit_events`.
   - Wrapper em `src/data/rpc.ts` e atualização de contratos em `src/types/schema.ts` e `src/types/database.ts`.
3. **Etapa 34.3 — Camada de Estado & TanStack Query:**
   - Hook de mutação `useImportBankTransactions` em `src/state/mutations/`.
   - Invalidação atômica e coordenada de cache: `useExpenses`, `useIncomes`, `useOverviewTotals`, `useBudgets`.
4. **Etapa 34.4 — Interface de Usuário Obsidian Glass em 3 Passos & Testes:**
   - Botão de ação "Importar extrato" no cabeçalho da página de Transações (`src/features/transactions/pages/transaction-list-page.tsx`).
   - Diálogo `BankStatementImportDialog` em `src/features/transactions/components/` com os 3 passos:
     - *Passo 1 (Upload):* Tabs com `Dropzone` para arquivos (OFX/CSV) e `Textarea` para colar texto.
     - *Passo 2 (Mapeamento):* Tabela de prévia com identificação assistida de colunas (Data, Descrição, Valor, Tipo D/C).
     - *Passo 3 (Conferência):* Tabela de conciliação bidirecional com badges de **Receita (+)** e **Despesa (-)**, filtros rápidos (*Todos*, *Despesas*, *Receitas*, *Novos*, *Conciliados*, *No App apenas*), seleção em lote, seletores de categoria/tipo e alerta de lançamentos manuais ausentes no extrato.
   - Testes de integração de UI (`bank-statement-import-dialog.test.tsx`).

**Arquivos:** `src/domain/reconciliation/*` (+ testes) · `supabase/migrations/20260101000016_bank_statement_import.sql` · `src/data/rpc.ts` · `src/types/schema.ts` · `src/types/database.ts` · `src/state/mutations/use-import-bank-transactions.ts` · `src/features/transactions/components/bank-statement-import-dialog.tsx` · `src/features/transactions/components/bank-statement-*-step.tsx` · `src/features/transactions/pages/transaction-list-page.tsx`.

**✅ DoD (critérios de aceite):**
- Motores puros com cobertura de testes unitários para múltiplos layouts bancários de conta corrente (Nubank, Itaú, Bradesco, Banco do Brasil, Inter, Caixa, OFX SGML/XML e Quick-Paste).
- Parsing e deduplicação ocorrem 100% no cliente sem envio de extratos bancários para servidores externos.
- Idempotência verificada: reimportar o mesmo extrato resulta em 0 duplicatas no banco; compras/recebimentos legítimos de mesmo valor/data no mesmo dia são preservados pelo índice ordinal.
- Zero double-counting de faturas de cartão e transferências/aportes: linhas identificadas vêm desmarcadas por padrão (`selected: false`).
- Reconciliação bidirecional: despesas e receitas manuais cadastradas no app que não vieram no extrato são listadas na aba "No App apenas" para auditoria.
...
### Fase 42 — Central Unificada de Relatórios de Consultoria Patrimonial & Exportação Institucional Multi-Abas (.xlsx / A4) (v1.80 — 2026-08-23)

> **Status:** 🟢 Concluído (2026-08-23) — Unificação e elevação da inteligência de relatórios do aplicativo para o padrão de Consultoria de Investimentos e Wealth Management. Consolidação de **todos** os relatórios e exportações em um único Hub em `/relatorios`, eliminando fragmentação com a área de investimentos e introduzindo 4 Dossiês Executivos A4, Caderno Excel Multi-Abas profissional (`.xlsx`) com fórmulas e análises de alocação/risco/cobertura patrimonial. Suíte 100% verde com 198 arquivos / 1.555 testes passando.
