...
> **Status:** v1.8 — **plano de execução canônico** do projeto (o `ESPECIFICACAO_TECNICA.md` §6 o referencia como resumo executivo). Foco em **ordem de execução**, **ordem de construção da UI (Design System primeiro)** e **Definition of Done (DoD)** por fase. **v1.63** registra o **Fallback de Cotações em Cascata (Brapi + AwesomeAPI) e Prevalência Estrita de Preço Manual na Carteira** (2026-08-18): **(1) Fallback em Cascata de Cotações**: Edge Function `supabase/functions/quotes` e motor puro `_shared/quotes-core.ts` atualizados com fallback em cascata caso o Yahoo Finance falhe/bloqueie — integração com a API **Brapi** (`brapi.dev`) para ações/FIIs/BDRs e com a **AwesomeAPI** (`economia.awesomeapi.com.br`) para câmbio (`USDBRL=X`), com novos parsers puros testados (`parseBrapiResponse`, `parseAwesomeApiResponse`, `normalizeTickerForBrapi`); **(2) Prevalência e Não-Zeramento de Preço Manual**: `listAssetPrices` em `src/data/repositories/asset-prices.ts` com `consolidateAssetPrices` garantindo que o override manual do usuário (`source: "manual"`) tenha precedência estrita sobre o cache global (`source: "api"`), impedindo que falhas de cotação sobrescrevam ou zerem os valores fixados pelo usuário; **(3) Derivação Segura em Posição da Carteira**: `usePortfolioPosition` blindado para extrair o `manualPrice` com prioridade máxima e usar o custo médio do ledger (`ledger.averageCost`) como fallback de segurança antes de qualquer valor estático. Suíte **1283 testes / 160 arquivos** (100% verde).
