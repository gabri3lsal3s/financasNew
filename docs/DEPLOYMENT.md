...
npm run test         # vitest — 1133 testes (143 arquivos)
...
- ✅ **Testes:** 1133 verdes (domínio puro, RPCs, telas, a11y com axe, teclado, deep-links, auditorias, atualizações otimistas, gestos).
...
| 2 | ~~**Deploy de produção**~~ | **✅ Funcional (confirmado 2026-08-15)** — frontend no Vercel + Supabase remoto; env vars configuradas. `deploy.yml` pronto para o CI/CD automatizado (gates + deploy condicional quando os secrets GitHub existirem). **2026-08-15 (fix):** o gate agora vive em `env` do job + `if` de **step** — o contexto `secrets` **não é permitido** em `if` de job (workflow inválido, todos os runs falhavam com 0 jobs); `vercel/vercel-action@v1` (deprecado, Node 16) trocado pela CLI oficial `npx vercel deploy --prod`. | GitHub → Settings → Secrets: `VERCEL_TOKEN`/`VERCEL_ORG_ID`/`VERCEL_PROJECT_ID`; `SUPABASE_ACCESS_TOKEN`/`SUPABASE_PROJECT_ID` |
...
[x] Deploy verde: typecheck + lint + 1133 testes + build — ✅ produção
