...
> **v1.3 (2026-08-17):** **(1) HTML Splash Screen Reativo** — `<div id="app-splash">` inline no `index.html` com suporte imediato aos 3 temas (`data-theme`), ícone 104px sem texto, animação pulse + barra de progresso teal; some automaticamente via `#root:not(:empty)`; **(2) Sincronização Dinâmica da Barra de Título** — sincronização em tempo real de `<meta name="theme-color">` no bootstrap (`index.html`) e no `ThemeProvider`, com suporte a `.app-region-drag` e `.app-region-no-drag` no header do `PageShell`; **(3) Ícones 100% Transparentes** — remoção completa de fundos brancos em todos os ícones maskable e PWA (`public/pwa/icons/`); **(4) `offline.html` redesenhado** — Inter, ícone 96px, botão discreto no tom do design system, suporte aos 3 temas, fade-in.
...
└── pwa/
    ...
    │   ├── maskable-192.png (transparente, safe zone 80%, purpose: "maskable")
    │   ├── maskable-512.png (transparente, safe zone 80%, purpose: "maskable")
