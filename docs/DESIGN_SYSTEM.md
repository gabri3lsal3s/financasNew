...
- **Estilo de superfície "Glass" (F11, padrão):** quando `data-surface-style` está ausente, o chrome — **modais, paleta de busca e painéis flutuantes** (`rounded-xl/2xl + bg-surface` com `z-modal`/`z-floating-tools`) — fica translúcido (`surface/82` + `backdrop-filter: blur(12px)`); o fundo do app (`body`) permanece limpo e uniforme com a cor de fundo padrão (`bg-background`), garantindo foco e zero distrações visuais ou halos. **Cards de dados permanecem opacos** (regra de legibilidade acima). `flat`/`elevated` (`data-surface-style`) mantêm o chrome opaco.
...
- **Regras Físicas A4 (`@page`) & CSS Paged Media:** margem de `10mm 12mm 12mm 12mm` com contadores de página nativos (`@bottom-right: "Página " counter(page) " de " counter(pages)` e `@bottom-left: "Guia Financeiro · Dossiê Oficial"`), ritmo vertical editorial fluido (`gap-4.5` a `gap-5` entre seções principais, `gap-2.5` a `gap-3` em sub-blocos, `p-4.5` / `print:p-4` em cards de síntese/risco e células de tabela com `py-1.5 px-2.5`).
- **Continuidade de Tabelas Multi-páginas:** cabeçalho identificador de classe de ativo colocalizado na primeira linha do `thead` da tabela (`display: table-header-group !important`), garantindo repetição contextual automática no topo de páginas divididas (sem tabelas órfãs). Linhas atômicas protegidas contra quebra interna (`tr { break-inside: avoid !important }`), títulos com `break-after: avoid !important`, e quebras seccionais inteligentes (`break-inside: auto`) nos gráficos de distribuição setorial.
- **Localização Estrita pt-BR & Zero Reticências:** todos os percentuais de relatórios utilizam máscara canônica `formatPercent()` (vírgula decimal pt-BR), com supressão de zero negativo (`-0,0%` -> `0,0%` via `formatSignedPct`). Legendas de distribuição em largura A4 adotam 2 colunas amplas com `print:truncate-none print:whitespace-normal`, assegurando legibilidade integral sem cortes elípticos (`…`).
- **Discriminação Fiscal Multi-Moeda (IRPF):** a Ficha de Bens e Direitos consome a classificação canônica de domínio (`getBensDireitosClassification`) e detecta a moeda do ativo: ativos em dólar (`USD`) têm seus custos unitário e total discriminados em `US$` com explicitação do valor equivalente em `R$`, respeitando as instruções oficiais da Receita Federal para declaração de bens no exterior.
- **Caderno Excel Multi-Abas & Blindagem CSV:** a exportação de relatórios em planilha (.xlsx/.xml) consolida 7 abas analíticas (incluindo *Metas e Rebalanceamento* com gaps e status de aportes). A função `downloadCsv` aplica defensivamente o BOM UTF-8 (`\uFEFF`) em todas as saídas, assegurando abertura sem corrupção de caracteres em qualquer versão do Microsoft Excel.
